...
const { ExternalContextAgent } = require('./src/lib/ai-agents/external-context-agent.ts');
...
async function testOpenAIIntegration() {
  ...
  try {
    ...
    if (result.success && result.output) {
      ...
      console.log(`📈 Analyse concurrentielle: ${result.output.competitor_analysis?.length || 0}`);
      console.log(`📅 Événements marché: ${result.output.market_events?.length || 0}`);
      console.log(`😊 Analyse sentiment: ${result.output.sentiment_analysis ? 'Oui' : 'Non'}`);

      if (result.output.market_insights && result.output.market_insights.length > 0) {
        console.log('\n🎯 Premier insight:');
        const firstInsight = result.output.market_insights[0];
        console.log(`   Titre: ${firstInsight.title}`);
        console.log(`   Impact: ${(firstInsight.impact_score * 100).toFixed(1)}%`);
