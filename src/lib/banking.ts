...
export class BankingService {
  ...
  constructor() {
    this.bridgeApiUrl = process.env.BRIDGE_API_URL || 'https://api.bridgeapi.io/v3';
  ...
  private async getBridgeAccessToken(): Promise<string> {
    try {
      ...
      const credentials = Buffer.from(`${this.bridgeClientId}:${this.bridgeClientSecret}`).toString('base64');
      ...
      if (!response.ok) {
        ...
        console.error('[BANKING_SERVICE] Bridge API error response:', errorText);
        throw new Error(`Bridge auth failed: ${response.status} ${response.statusText}`);
      }
  ...
  async getUserBankAccounts(clerkUserId: string): Promise<any[]> {
    try {
      ...
      if (!user) {
        console.log(`[BANKING_SERVICE] No user found for Clerk ID: ${clerkUserId}`);
        return [];
      }
      ...
      console.log(`[BANKING_SERVICE] Found ${accounts.length} accounts for user ${user.id} (Clerk: ${clerkUserId})`);
      return accounts;
    } catch (error) {
  ...
  async getUserBankTransactions(clerkUserId: string, accountId?: string, limit: number = 50): Promise<any[]> {
    try {
      // Récupérer l'ID utilisateur interne depuis Clerk ID
      const user = await prisma.user.findUnique({
        where: { clerkId: clerkUserId },
      ...
      console.log(`[BANKING_SERVICE] Found ${transactions.length} transactions for user ${user.id} (Clerk: ${clerkUserId})`);
      return transactions;
    } catch (error) {
  ...
  async syncUserBankData(userId: string): Promise<{ accounts: number; transactions: number }> {
    try {
      ...
      for (const bridgeAccount of accountsData.resources || []) {
        ...
        const transactionCount = await this.syncAccountTransactions(userId, bridgeAccount.id, accessToken);
  ...
  private async syncBankAccount(userId: string, bridgeAccount: BridgeAccountData): Promise<void> {
    try {
  ...
  private async syncAccountTransactions(userId: string, bridgeAccountId: string, accessToken: string): Promise<number> {
    try {
      ...
      if (!localAccount) {
        console.warn(`[BANKING_SERVICE] Local account not found for Bridge ID: ${bridgeAccountId}`);
        return 0;
      }

      const transactionsResponse = await fetch(`${this.bridgeApiUrl}/accounts/${bridgeAccountId}/transactions?limit=100`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
        },
      ...
      for (const bridgeTransaction of transactionsData.resources || []) {
        await this.syncBankTransaction(userId, localAccount.id, bridgeTransaction);
        syncedCount++;
      }
  ...
  private async syncBankTransaction(userId: string, accountId: string, bridgeTransaction: BridgeTransactionData): Promise<void> {
    try {
  ...
  async getFinancialAnalytics(clerkUserId: string, timeframe: string = '30d'): Promise<any> {
    try {
      // Récupérer l'ID utilisateur interne depuis Clerk ID
      const user = await prisma.user.findUnique({
        where: { clerkId: clerkUserId },
  ...
  private calculateMonthlyTrend(transactions: any[]): Record<string, { income: number; expenses: number }> {
