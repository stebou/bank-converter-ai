...
export class StockOptimizationAgent extends BaseAgent {
  ...
  async execute(input: StockOptimizationInput, state: StockAnalysisState): Promise<AgentExecutionResult> {
    ...
    try {
      ...
      const reorderPoints = await this.calculateReorderPoints(input, safetyStockResults);

      // 4. Optimisation par segmentation ABC/XYZ
      const segmentOptimization = await this.optimizeBySegmentation(input, state);

      // 5. Analyse de sensibilité
      const sensitivityAnalysis = await this.performSensitivityAnalysis(input, eoqResults);

      // 6. Recommandations d'amélioration
      const improvementRecommendations = await this.generateImprovementRecommendations(
        eoqResults, safetyStockResults, reorderPoints
      );
      ...
      return {
        ...
        success: true,
        confidence_score: this.calculateOptimizationConfidence(eoqResults, safetyStockResults),
        output: {
          ...
          optimization_summary: this.generateOptimizationSummary(eoqResults, safetyStockResults)
        },
        metrics: {
  ...
  private async calculateEOQForAllProducts(input: StockOptimizationInput): Promise<EOQResult[]> {
  ...
  private async calculateEOQForProduct(pattern: DemandPattern, input: StockOptimizationInput): Promise<EOQResult> {
    // Calcul de la demande annuelle à partir de l'historique
    const productSales = input.sales_history.filter(sale => sale.product_id === pattern.product_id);
    const annualDemand = this.calculateAnnualDemand(productSales);

    if (annualDemand <= 0) {
      throw new Error(`Invalid annual demand for product ${pattern.product_id}`);
    }
    ...
    const holdingCostPerUnit = avgUnitPrice * input.cost_parameters.holding_cost_rate;

    // Formule EOQ: sqrt((2 * D * S) / H)
    // D = Demande annuelle, S = Coût de commande, H = Coût de stockage unitaire
    const eoq = Math.sqrt(
      (2 * annualDemand * input.cost_parameters.ordering_cost) / holdingCostPerUnit
    );
    ...
    // Coût total annuel
    const orderingCostAnnual = orderFrequency * input.cost_parameters.ordering_cost;
  ...
  private async calculateSafetyStocks(input: StockOptimizationInput): Promise<SafetyStock[]> {
    const results: SafetyStock[] = [];

    for (const pattern of input.demand_patterns) {
      try {
        const safetyStock = await this.calculateSafetyStockForProduct(pattern, input);
  ...
  private async calculateSafetyStockForProduct(pattern: DemandPattern, input: StockOptimizationInput): Promise<SafetyStock> {
    const productSales = input.sales_history.filter(sale => sale.product_id === pattern.product_id);
    
    // Calcul de la demande moyenne et écart-type durant le délai d'approvisionnement
    const demandStats = this.calculateDemandStatistics(productSales, input.lead_time_days);
    ...
    const adjustedSafetyStock = Math.ceil(safetyStockQuantity * volatilityAdjustment);

    // Calcul du coût du stock de sécurité
    const avgUnitPrice = this.calculateAverageUnitPrice(productSales);
    const safetyStockCost = adjustedSafetyStock * avgUnitPrice * input.cost_parameters.holding_cost_rate;
  ...
  private async calculateReorderPoints(input: StockOptimizationInput, safetyStocks: SafetyStock[]): Promise<ReorderPoint[]> {
    const results: ReorderPoint[] = [];

    for (const safetyStock of safetyStocks) {
      try {
        const reorderPoint = await this.calculateReorderPointForProduct(safetyStock, input);
  ...
  private async calculateReorderPointForProduct(safetyStock: SafetyStock, input: StockOptimizationInput): Promise<ReorderPoint> {
    const productSales = input.sales_history.filter(sale => sale.product_id === safetyStock.product_id);
    ...
    const reorderPointQuantity = Math.ceil(leadTimeDemand + safetyStock.safety_stock_quantity);
    ...
    return {
      ...
      recommended_order_quantity: this.findEOQForProduct(safetyStock.product_id, input),
  ...
  private async optimizeBySegmentation(input: StockOptimizationInput, state: StockAnalysisState): Promise<any> {
    ...
    for (const segment of state.processed_insights.product_segments) {
      ...
      if (!segmentStrategies[segmentKey]) {
        segmentStrategies[segmentKey] = {
          segment: segmentKey,
          products: [],
          strategy: this.getOptimizationStrategy(segment.abc_classification, segment.xyz_classification),
          recommended_parameters: this.getRecommendedParameters(segment.abc_classification, segment.xyz_classification)
        };
      }
  ...
  private async performSensitivityAnalysis(input: StockOptimizationInput, eoqResults: EOQResult[]): Promise<any> {
    const sensitivities = [];
    ...
    for (const variation of orderingCostVariations) {
      ...
      const modifiedEOQ = await this.calculateEOQForProduct(input.demand_patterns[0], modifiedInput);
      const originalEOQ = eoqResults[0];

      const eoqChange = ((modifiedEOQ.economic_order_quantity - originalEOQ.economic_order_quantity) / originalEOQ.economic_order_quantity) * 100;
    ...
    return {
      sensitivities,
      most_sensitive_parameter: this.identifyMostSensitiveParameter(sensitivities),
  ...
  private calculateDemandStatistics(sales: SalesRecord[], leadTimeDays: number): { leadTimeMean: number; leadTimeStdDev: number } {
  ...
  private calculateVolatilityAdjustment(pattern: DemandPattern): number {
    ...
    } else if (pattern.pattern_type === 'SEASONAL' && pattern.seasonality_strength > 0.7) {
  ...
  private findEOQForProduct(productId: string, input: StockOptimizationInput): number {
  ...
  private getRecommendedParameters(abc: string, xyz: string): any {
    const parameters: { [key: string]: any } = {
      'A-X': { serviceLevel: 0.99, reviewFrequency: 'daily', safetyStockMultiplier: 0.8 },
      'A-Y': { serviceLevel: 0.97, reviewFrequency: 'weekly', safetyStockMultiplier: 1.0 },
      'A-Z': { serviceLevel: 0.95, reviewFrequency: 'weekly', safetyStockMultiplier: 1.4 },
      'B-X': { serviceLevel: 0.95, reviewFrequency: 'weekly', safetyStockMultiplier: 1.0 },
      'B-Y': { serviceLevel: 0.93, reviewFrequency: 'bi-weekly', safetyStockMultiplier: 1.1 },
      'B-Z': { serviceLevel: 0.90, reviewFrequency: 'bi-weekly', safetyStockMultiplier: 1.3 },
      'C-X': { serviceLevel: 0.90, reviewFrequency: 'monthly', safetyStockMultiplier: 0.9 },
      'C-Y': { serviceLevel: 0.87, reviewFrequency: 'monthly', safetyStockMultiplier: 1.0 },
      'C-Z': { serviceLevel: 0.85, reviewFrequency: 'quarterly', safetyStockMultiplier: 1.2 }
    };

    return parameters[`${abc}-${xyz}`] || { serviceLevel: 0.95, reviewFrequency: 'weekly', safetyStockMultiplier: 1.0 };
  }
  ...
  private identifyMostSensitiveParameter(sensitivities: any[]): string {
    return sensitivities.reduce((prev, curr) => 
      Math.abs(curr.sensitivity_ratio) > Math.abs(prev.sensitivity_ratio) ? curr : prev
    ).parameter;
  }
  ...
  private generateSensitivityRecommendations(sensitivities: any[]): string[] {
    ...
    for (const sensitivity of sensitivities) {
      if (Math.abs(sensitivity.sensitivity_ratio) > 0.5) {
        recommendations.push(`Attention: ${sensitivity.parameter} est très sensible (ratio: ${sensitivity.sensitivity_ratio.toFixed(2)})`);
      }
    }
  ...
  private async generateImprovementRecommendations(eoqResults: EOQResult[], safetyStocks: SafetyStock[], reorderPoints: ReorderPoint[]): Promise<string[]> {
    const recommendations = [];

    // Analyse des EOQ trop élevées
    const highEOQ = eoqResults.filter(eoq => eoq.economic_order_quantity > eoq.annual_demand * 0.3);
    if (highEOQ.length > 0) {
      recommendations.push(`${highEOQ.length} produits ont un EOQ élevé (>30% demande annuelle) - réviser coûts de commande`);
    }

    // Analyse des stocks de sécurité élevés
    const highSafetyStock = safetyStocks.filter(ss => ss.volatility_adjustment > 1.3);
    if (highSafetyStock.length > 0) {
      recommendations.push(`${highSafetyStock.length} produits nécessitent des stocks de sécurité élevés - améliorer prévisions`);
    }

    // Analyse des points de commande critiques
    const criticalReorder = reorderPoints.filter(rp => rp.confidence_score < 0.7);
    if (criticalReorder.length > 0) {
      recommendations.push(`${criticalReorder.length} points de commande ont faible confiance - vérifier données historiques`);
    }

    return recommendations;
  }

  private calculateOptimizationConfidence(eoqResults: EOQResult[], safetyStocks: SafetyStock[]): number {
    if (eoqResults.length === 0 || safetyStocks.length === 0) return 0;

    const avgSafetyStockConfidence = safetyStocks.reduce((sum, ss) => sum + ss.confidence_score, 0) / safetyStocks.length;
    const eoqValidityScore = eoqResults.filter(eoq => eoq.economic_order_quantity > 0 && eoq.annual_demand > 0).length / eoqResults.length;
  ...
  private calculateAccuracy(eoqResults: EOQResult[]): number {
    ...
    const validResults = eoqResults.filter(eoq => 
  ...
  private generateOptimizationSummary(eoqResults: EOQResult[], safetyStocks: SafetyStock[]): string {
    const totalProducts = eoqResults.length;
    const avgEOQ = eoqResults.reduce((sum, eoq) => sum + eoq.economic_order_quantity, 0) / totalProducts;
    const totalAnnualCost = eoqResults.reduce((sum, eoq) => sum + eoq.total_annual_cost, 0);
    const avgSafetyStock = safetyStocks.reduce((sum, ss) => sum + ss.safety_stock_quantity, 0) / safetyStocks.length;

    return `Optimisation de ${totalProducts} produits: EOQ moyen ${Math.round(avgEOQ)} unités, ` +
