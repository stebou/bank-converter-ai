...
class DataRetrievalAgent {
  async execute(userId: string): Promise<any> {
    ...
    try {
      ...
      const enrichedData = {
        ...
        documentTransactions: user?.documents.flatMap(doc => doc.transactions) || [],
  ...
  private assessDataQuality(accounts: any[], transactions: any[]): {
    score: number;
    completeness: number;
    recency: number;
    consistency: number;
    ...
    if (recentTransactions.length > 0) recency = Math.min(100, (recentTransactions.length / transactions.length) * 100);
...
class VerificationAgent {
  async execute(data: any): Promise<any> {
    console.log('[VERIFICATION_AGENT] Starting data verification and classification');
  ...
  "recurringPatterns": [
    {
      ...
      if (verificationResult.error) {
        console.error('[VERIFICATION_AGENT] Received fallback result due to JSON parsing error');
        throw new Error(`Verification failed: ${verificationResult.originalError}`);
      }
      
      console.log('[VERIFICATION_AGENT] Completed with reliability score:', verificationResult.dataReliability?.score);
  ...
  "opportunities": [
    {
      ...
      if (analysisResult.error) {
        console.error('[ANALYSIS_AGENT] Received fallback result due to JSON parsing error');
        throw new Error(`AI analysis failed: ${analysisResult.originalError}`);
      }
      
      console.log('[ANALYSIS_AGENT] Completed with health score:', analysisResult.financialHealth?.overallScore);
...
class ReportingAgent {
  async execute(data: any, verification: any, analysis: any): Promise<BusinessInsightReport> {
  ...
  "recommendations": [
    {
      ...
      // Vérifier si c'est un résultat de fallback
      if (reportData.error) {
        console.error('[REPORTING_AGENT] Received fallback result due to JSON parsing error');
        throw new Error(`Report generation failed: ${reportData.originalError}`);
      }
...
export class AIAgentOrchestrator {
  ...
  async executeWorkflow(userId: string): Promise<AgentWorkflow> {
    ...
    try {
      ...
      const verificationTask = workflow.tasks.find(t => t.type === 'verification')!;
      ...
      const report = await this.reportingAgent.execute(data, verification, analysis);
      ...
      if (runningTask) {
        ...
        runningTask.error = error instanceof Error ? error.message : String(error);
...
function safeJsonParse(jsonString: string): any {
  try {
    ...
    console.error('[SAFE_JSON_PARSE] Original string (first 500 chars):', jsonString.substring(0, 500));
    console.error('[SAFE_JSON_PARSE] Cleaned string (first 500 chars):', cleanResponse?.substring(0, 500));
