...
export class DataAnalysisAgent extends BaseAgent {
  ...
  async execute(input: DataAnalysisInput, state: StockAnalysisState): Promise<AgentExecutionResult> {
    ...
    try {
      ...
      const demandPatterns = await this.analyzeDemandPatterns(input.sales_history);
      
      // Segmentation ABC/XYZ des produits
      const productSegments = await this.segmentProducts(input.sales_history);
      
      // Analyse de saisonnalité
      const seasonalityData = await this.analyzeSeasonality(input.sales_history);
      ...
      const keyInsights = this.generateKeyInsights(demandPatterns, productSegments, seasonalityData);
      ...
      const output: DataAnalysisOutput = {
        ...
        confidence_score: this.calculateOverallConfidence(demandPatterns, dataQualityScore),
      ...
      const executionTime = Date.now() - startTime;
      this.log('error', 'Data analysis failed', { error: error instanceof Error ? error.message : error });
      throw error;
    }
  }

  // Analyse des patterns de demande pour chaque produit
  private async analyzeDemandPatterns(salesHistory: SalesRecord[]): Promise<DemandPattern[]> {
  ...
  private async analyzeProductPattern(productId: string, sales: SalesRecord[]): Promise<DemandPattern> {
    ...
    const patternType = this.classifyPattern(volatility, seasonalityStrength, trendAnalysis.strength);
    
    // Calcul de confiance basé sur la quantité et qualité des données
    const confidence = this.calculatePatternConfidence(sales.length, volatility);
  ...
  // Segmentation ABC/XYZ des produits
  private async segmentProducts(salesHistory: SalesRecord[]): Promise<ProductSegment[]> {
  ...
  // Analyse de saisonnalité
  private async analyzeSeasonality(salesHistory: SalesRecord[]): Promise<SeasonalityData[]> {
    ...
    for (const [productId, sales] of productSales.entries()) {
      ...
      const seasonalMultipliers = this.calculateSeasonalMultipliers(monthlySales);
  ...
  private groupSalesByProduct(sales: SalesRecord[]): Map<string, SalesRecord[]> {
  ...
  private calculateAutocorrelation(values: number[], lag: number): number {
    ...
    return Math.abs(correlation / n - mean1 * mean2) / (this.calculateStandardDeviation(values) ** 2);
  }

  private detectTrend(values: number[]): { direction: 'UP' | 'DOWN' | 'STABLE'; strength: number } {
  ...
  private calculateProductMetrics(salesHistory: SalesRecord[]): Map<string, any> {
  ...
  private performABCClassification(productMetrics: Map<string, any>): Map<string, 'A' | 'B' | 'C'> {
    const classification = new Map<string, 'A' | 'B' | 'C'>();
    
    // Trier par revenue total
    const sortedProducts = Array.from(productMetrics.entries())
      .sort(([,a], [,b]) => b.totalRevenue - a.totalRevenue);
    
    const totalRevenue = sortedProducts.reduce((sum, [, metrics]) => sum + metrics.totalRevenue, 0);
  ...
  private performXYZClassification(productMetrics: Map<string, any>): Map<string, 'X' | 'Y' | 'Z'> {
  ...
  private classifyVelocity(avgMonthlySales: number): 'FAST' | 'MEDIUM' | 'SLOW' {
  ...
  private calculateSeasonalMultipliers(monthlySales: Map<number, number>): number[] {
    const multipliers = new Array(12).fill(1);
    const avgMonthlySales = this.calculateMean(Array.from(monthlySales.values()));
  ...
  private identifyPeakLowSeasons(multipliers: number[]): { peak: { start: number; end: number }; low: { start: number; end: number } } {
  ...
  private identifySeasonalPeriods(monthlySales: Map<number, number>): number[] {
    ...
    for (const [month, sales] of monthlySales.entries()) {
      if (sales > avgSales * 1.2) { // 20% au-dessus de la moyenne
  ...
  private calculateDataQuality(salesHistory: SalesRecord[]): number {
    ...
    return (completenessScore * 0.4 + consistencyScore * 0.4 + freshnessScore * 0.2);
  }
  ...
  private calculateConsistencyScore(salesHistory: SalesRecord[]): number {
    ...
    for (const record of salesHistory) {
      if (record.quantity_sold > 0 && record.revenue > 0 && record.product_id && record.date) {
        consistentRecords++;
      }
    }
  ...
  private calculateFreshnessScore(salesHistory: SalesRecord[]): number {
    ...
    const daysSinceLastSale = (now.getTime() - mostRecentSale) / (1000 * 60 * 60 * 24);
  ...
  private generateKeyInsights(
    ...
    const seasonalProducts = patterns.filter(p => p.pattern_type === 'SEASONAL').length;
    const erraticProducts = patterns.filter(p => p.pattern_type === 'ERRATIC').length;
    
    if (seasonalProducts > 0) {
      insights.push(`${seasonalProducts} produits présentent des patterns saisonniers forts`);
    }
    
    if (erraticProducts > patterns.length * 0.3) {
      insights.push(`${erraticProducts} produits ont des patterns de demande erratiques nécessitant une attention particulière`);
    }
    
    // Insights sur la segmentation
    const criticalProducts = segments.filter(s => s.strategic_importance === 'CRITICAL').length;
    const aProducts = segments.filter(s => s.abc_classification === 'A').length;
    
    insights.push(`${aProducts} produits génèrent 80% du chiffre d'affaires (classe A)`);
    insights.push(`${criticalProducts} produits sont identifiés comme critiques pour l'activité`);
    
    // Insights sur la saisonnalité
    if (seasonality.length > 0) {
      const avgSeasonalStrength = patterns
        .map(p => p.seasonality_strength)
        .reduce((a, b) => a + b, 0) / patterns.length;
      
      if (avgSeasonalStrength > 0.3) {
        insights.push('Fort impact saisonnier détecté - optimisation des stocks par saison recommandée');
      }
    }
  ...
  private calculateOverallConfidence(patterns: DemandPattern[], dataQualityScore: number): number {
