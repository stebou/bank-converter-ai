...
export class KPIMetricsAgent extends BaseAgent {
  ...
  async execute(input: KPICalculationInput, state: StockAnalysisState): Promise<AgentExecutionResult> {
    ...
    try {
      ...
      const operationalKPIs = await this.calculateOperationalExcellence(input, state);

      // 3. Calcul des KPIs de performance financière
      const financialKPIs = await this.calculateFinancialPerformance(input, state);

      // 4. Calcul des KPIs de satisfaction client
      const customerKPIs = await this.calculateCustomerSatisfaction(input, state);

      // 5. Calcul des KPIs d'agilité supply chain
      const supplyChainKPIs = await this.calculateSupplyChainAgility(input, state);

      // 6. Calcul des KPIs de durabilité
      const sustainabilityKPIs = await this.calculateSustainabilityMetrics(input, state);

      // 7. Calcul des KPIs de performance IA
      const aiPerformanceKPIs = await this.calculateAISystemPerformance(input, state);
      ...
      const benchmarkComparisons = await this.performBenchmarkComparison(extendedKPIs, input.benchmark_data);

      // 11. Identification des KPIs critiques
      const criticalKPIs = this.identifyCriticalKPIs(extendedKPIs, trendAnalysis);
      ...
      const improvementRecommendations = await this.generateImprovementRecommendations(
        extendedKPIs, trendAnalysis, benchmarkComparisons
      );
      ...
      return {
        ...
        metrics: {
          ...
          processing_speed: this.countTotalKPIs(extendedKPIs) / executionTime * 1000,
  ...
  private async calculateBaseKPIs(input: KPICalculationInput, state: StockAnalysisState): Promise<StockManagementKPIs> {
    const forecastAccuracy = this.calculateEnhancedForecastAccuracy(input.forecasts, input.sales_history);
    const serviceMetrics = this.calculateEnhancedServiceMetrics(input, state);
    const financialMetrics = this.calculateEnhancedFinancialMetrics(input, state);
  ...
  private async calculateOperationalExcellence(input: KPICalculationInput, state: StockAnalysisState) {
    return {
      perfect_order_rate: this.calculatePerfectOrderRate(input.sales_history),
      on_time_delivery_rate: this.calculateOnTimeDeliveryRate(input.sales_history),
      order_fill_rate: this.calculateOrderFillRate(input.sales_history, input.current_inventory),
      inventory_accuracy: this.calculateInventoryAccuracy(input.current_inventory),
      cycle_count_accuracy: this.calculateCycleCountAccuracy(input.current_inventory)
    };
  }

  // KPIs de performance financière
  private async calculateFinancialPerformance(input: KPICalculationInput, state: StockAnalysisState) {
    const gmroi = this.calculateGMROI(input.sales_history, input.current_inventory);
    const inventoryToSalesRatio = this.calculateInventoryToSalesRatio(input.sales_history, input.current_inventory);
    const carryingCost = this.calculateCarryingCostOfInventory(input.current_inventory);
    const obsoletePercentage = this.calculateObsoleteInventoryPercentage(input.current_inventory, input.sales_history);
    const cashToCashCycle = this.calculateCashToCashCycleTime(input.sales_history);
  ...
  private async calculateCustomerSatisfaction(input: KPICalculationInput, state: StockAnalysisState) {
    return {
      ...
      customer_complaint_rate: this.calculateCustomerComplaintRate(input.sales_history),
      return_rate: this.calculateReturnRate(input.sales_history),
      lead_time_variability: this.calculateLeadTimeVariability(input.sales_history)
    };
  }

  // KPIs d'agilité supply chain
  private async calculateSupplyChainAgility(input: KPICalculationInput, state: StockAnalysisState) {
    return {
      supplier_performance_score: this.calculateSupplierPerformanceScore(state.raw_data.supplier_data || []),
      demand_forecast_accuracy: this.calculateDemandForecastAccuracy(input.forecasts, input.sales_history),
      supply_plan_accuracy: this.calculateSupplyPlanAccuracy(input.current_inventory),
      inventory_velocity: this.calculateInventoryVelocity(input.sales_history, input.current_inventory),
      supply_chain_cycle_time: this.calculateSupplyChainCycleTime(state.raw_data.supplier_data || [])
    };
  }

  // KPIs de durabilité
  private async calculateSustainabilityMetrics(input: KPICalculationInput, state: StockAnalysisState) {
    return {
      waste_reduction_percentage: this.calculateWasteReduction(input.current_inventory),
  ...
  private async calculateAISystemPerformance(input: KPICalculationInput, state: StockAnalysisState) {
    return {
      model_accuracy_trend: this.calculateModelAccuracyTrend(input.forecasts),
      prediction_reliability: this.calculatePredictionReliability(input.forecasts, input.sales_history),
      automation_rate: this.calculateAutomationRate(state),
      decision_support_effectiveness: this.calculateDecisionSupportEffectiveness(state.recommendations),
  ...
  private calculateOrderFillRate(sales: SalesRecord[], inventory: any[]): number {
    const totalDemand = sales.reduce((sum, sale) => sum + sale.quantity_sold, 0);
    const totalInventory = inventory.reduce((sum, inv) => sum + inv.available_quantity, 0);
  ...
  private calculateGMROI(sales: SalesRecord[], inventory: any[]): number {
    const grossMargin = sales.reduce((sum, sale) => sum + sale.revenue * 0.3, 0); // 30% de marge
    const avgInventoryValue = inventory.reduce((sum, inv) => sum + inv.available_quantity * 25, 0);
    return avgInventoryValue > 0 ? (grossMargin / avgInventoryValue) * 100 : 0;
  }

  private calculateInventoryToSalesRatio(sales: SalesRecord[], inventory: any[]): number {
    const monthlySales = this.calculateMonthlySales(sales);
    const inventoryValue = inventory.reduce((sum, inv) => sum + inv.available_quantity * 25, 0);
  ...
  private calculateObsoleteInventoryPercentage(inventory: any[], sales: SalesRecord[]): number {
  ...
  private calculateSupplierPerformanceScore(suppliers: any[]): number {
    ...
    const avgReliability = suppliers.reduce((sum, sup) => sum + sup.reliability_score, 0) / suppliers.length;
    return avgReliability * 100;
  }

  private calculateDemandForecastAccuracy(forecasts: ForecastResult[], sales: SalesRecord[]): number {
    if (forecasts.length === 0) return 0;
    const avgAccuracy = forecasts.reduce((sum, forecast) => sum + forecast.accuracy_score, 0) / forecasts.length;
  ...
  private calculateInventoryVelocity(sales: SalesRecord[], inventory: any[]): number {
  ...
  private calculateSupplyChainCycleTime(suppliers: any[]): number {
    ...
    const avgLeadTime = suppliers.reduce((sum, sup) => sum + sup.lead_time_days, 0) / suppliers.length;
    return avgLeadTime;
  }

  // Méthodes de durabilité (simulées)
  private calculateWasteReduction(): number { return 12.5; }
  private calculateEnergyEfficiency(): number { return 78.3; }
  private calculatePackagingOptimization(): number { return 85.1; }
  private calculateTransportEfficiency(): number { return 82.7; }
  ...
  private calculateModelAccuracyTrend(forecasts: ForecastResult[]): number {
    ...
    const recentAccuracy = accuracies.slice(-10).reduce((sum, acc) => sum + acc, 0) / 10;
    const previousAccuracy = accuracies.slice(-20, -10).reduce((sum, acc) => sum + acc, 0) / 10;
    return ((recentAccuracy - previousAccuracy) / previousAccuracy) * 100;
  }

  private calculatePredictionReliability(forecasts: ForecastResult[], sales: SalesRecord[]): number {
  ...
  private calculateDecisionSupportEffectiveness(recommendations: any[]): number {
  ...
  private async analyzeTrends(currentKPIs: ExtendedKPIs): Promise<KPITrend[]> {
    ...
    const kpiComparisons = [
      { name: 'forecast_accuracy', current: currentKPIs.forecast_accuracy.overall_mape, previous: previousKPIs.forecast_accuracy.overall_mape, inverse: true },
      { name: 'service_level', current: currentKPIs.service_metrics.service_level, previous: previousKPIs.service_metrics.service_level },
      { name: 'inventory_turnover', current: currentKPIs.financial_metrics.inventory_turnover, previous: previousKPIs.financial_metrics.inventory_turnover },
      { name: 'perfect_order_rate', current: currentKPIs.operational_excellence.perfect_order_rate, previous: previousKPIs.operational_excellence.perfect_order_rate }
    ];

    for (const comparison of kpiComparisons) {
      const percentageChange = ((comparison.current - comparison.previous) / comparison.previous) * 100;
      const isImproving = comparison.inverse ? percentageChange < 0 : percentageChange > 0;
      
      trends.push({
        kpi_name: comparison.name,
        current_value: comparison.current,
        previous_period_value: comparison.previous,
        trend_direction: Math.abs(percentageChange) < 2 ? 'STABLE' : (isImproving ? 'IMPROVING' : 'DECLINING'),
        percentage_change: percentageChange,
        significance_level: Math.abs(percentageChange) > 10 ? 'HIGH' : (Math.abs(percentageChange) > 5 ? 'MEDIUM' : 'LOW')
      });
    }
  ...
  private async performBenchmarkComparison(kpis: ExtendedKPIs, benchmarkData?: any): Promise<BenchmarkComparison[]> {
    if (!benchmarkData) {
    ...
    for (const comparison of benchmarkComparisons) {
      comparisons.push({
        ...
        performance_vs_industry: comparison.current > comparison.industry ? 'ABOVE' : 
                                (comparison.current < comparison.industry ? 'BELOW' : 'AT_LEVEL'),
        performance_vs_target: comparison.current > comparison.target ? 'ABOVE' : 
                              (comparison.current < comparison.target ? 'BELOW' : 'AT_TARGET'),
        improvement_potential: Math.max(0, comparison.target - comparison.current)
      });
    }
  ...
  private identifyCriticalKPIs(kpis: ExtendedKPIs, trends: KPITrend[]): string[] {
    const criticalKPIs = [];

    // KPIs critiques basés sur les seuils
    if (kpis.service_metrics.service_level < 90) criticalKPIs.push('Service Level');
    if (kpis.forecast_accuracy.overall_mape > 25) criticalKPIs.push('Forecast Accuracy');
    if (kpis.financial_metrics.inventory_turnover < 4) criticalKPIs.push('Inventory Turnover');
    if (kpis.operational_excellence.perfect_order_rate < 85) criticalKPIs.push('Perfect Order Rate');

    // KPIs critiques basés sur les tendances
    const decliningTrends = trends.filter(t => t.trend_direction === 'DECLINING' && t.significance_level === 'HIGH');
  ...
  private async generateImprovementRecommendations(
    ...
    if (kpis.service_metrics.service_level < 90) {
      recommendations.push('Améliorer le niveau de service: réviser les stocks de sécurité et optimiser les points de commande');
    }

    if (kpis.forecast_accuracy.overall_mape > 25) {
      recommendations.push('Améliorer la précision des prévisions: intégrer plus de facteurs externes et recalibrer les modèles');
    }

    // Recommandations basées sur les tendances
    const decliningTrends = trends.filter(t => t.trend_direction === 'DECLINING');
    for (const trend of decliningTrends) {
      recommendations.push(`Attention: ${trend.kpi_name} en baisse de ${Math.abs(trend.percentage_change).toFixed(1)}% - investigation requise`);
    }

    // Recommandations basées sur les benchmarks
    const underperformingKPIs = benchmarks.filter(b => b.performance_vs_industry === 'BELOW');
    for (const kpi of underperformingKPIs) {
      recommendations.push(`${kpi.kpi_name} sous la moyenne industrie: potentiel d'amélioration de ${kpi.improvement_potential.toFixed(1)} points`);
    }
  ...
  private calculateEnhancedForecastAccuracy(forecasts: ForecastResult[], sales: SalesRecord[]): StockManagementKPIs['forecast_accuracy'] {
    if (forecasts.length === 0) {
      return { overall_mape: 100, short_term_mape: 100, medium_term_mape: 100, bias_percentage: 0 };
    }

    const overallMape = forecasts.reduce((sum, f) => sum + (1 - f.accuracy_score) * 100, 0) / forecasts.length;
  ...
  private calculateEnhancedServiceMetrics(input: KPICalculationInput, state: StockAnalysisState): StockManagementKPIs['service_metrics'] {
    const stockoutAlerts = input.alerts.filter(a => a.type === 'STOCKOUT_RISK');
    const totalProducts = input.current_inventory.length;

    return {
      service_level: Math.max(0, 100 - (stockoutAlerts.length / Math.max(totalProducts, 1)) * 100),
      fill_rate: this.calculateOrderFillRate(input.sales_history, input.current_inventory),
      stockout_frequency: stockoutAlerts.filter(a => a.severity === 'CRITICAL').length,
      average_stockout_duration: 2.1 // Simulation
    };
  }

  private calculateEnhancedFinancialMetrics(input: KPICalculationInput, state: StockAnalysisState): StockManagementKPIs['financial_metrics'] {
    return {
      inventory_turnover: this.calculateInventoryVelocity(input.sales_history, input.current_inventory) / 365 * 12,
      days_of_inventory: this.calculateDaysOfInventory(input.sales_history, input.current_inventory),
      holding_cost_percentage: 15, // 15% annuel
      dead_stock_ratio: this.calculateObsoleteInventoryPercentage(input.current_inventory, input.sales_history)
    };
  }

  private calculateEnhancedAIPerformance(input: KPICalculationInput, state: StockAnalysisState): StockManagementKPIs['ai_performance'] {
    return {
  ...
  private calculateDaysOfInventory(sales: SalesRecord[], inventory: any[]): number {
    const dailySales = this.calculateMonthlySales(sales) / 30;
    const inventoryValue = inventory.reduce((sum, inv) => sum + inv.available_quantity * 25, 0);
    return dailySales > 0 ? inventoryValue / dailySales : 0;
  }

  private calculateBias(forecasts: ForecastResult[], sales: SalesRecord[]): number {
  ...
  private generatePerformanceSummary(kpis: ExtendedKPIs): string {
    ...
    const forecastAccuracy = (100 - kpis.forecast_accuracy.overall_mape).toFixed(1);
    const inventoryTurnover = kpis.financial_metrics.inventory_turnover.toFixed(1);
    
    return `Performance globale: Service ${serviceLevel}%, Prévisions ${forecastAccuracy}%, Rotation ${inventoryTurnover}x. ` +
  ...
  private prepareDashboardMetrics(kpis: ExtendedKPIs): any {
    return {
      ...
      financial_health: {
        ...
        obsolete_percentage: kpis.financial_performance.obsolete_inventory_percentage
      },
      ai_performance: {
        automation_rate: kpis.ai_system_performance.automation_rate,
        prediction_reliability: kpis.ai_system_performance.prediction_reliability,
