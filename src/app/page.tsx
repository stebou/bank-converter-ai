...
import { Upload, FileText, Brain, CheckCircle, AlertCircle, Download, Loader2, Zap, Target, TrendingUp, Check, Crown, Sparkles, ArrowRight, Menu, X, Mail, Phone, MapPin, Twitter, Linkedin, Github, Facebook } from 'lucide-react';
import Link from 'next/link'; 
import { SignedIn, SignedOut, UserButton } from '@clerk/nextjs';

// Navigation Component (mis à jour avec Clerk)
const Navigation = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isScrolled, setIsScrolled] = useState(false);

  // Detect scroll pour l'effet glassmorphism
  React.useEffect(() => {
    const handleScroll = () => {
  ...
  const headerClasses = `fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-out ${
    isScrolled 
      ? 'bg-gradient-to-r from-blue-600/90 to-purple-600/90 backdrop-blur-md border-b border-white/20' 
      : 'bg-transparent'
  }`;

  const titleClasses = `text-lg font-bold transition-colors duration-300 ${
    isScrolled ? 'text-white' : 'text-gray-900'
  }`;

  const navLinkClasses = `font-medium transition-colors duration-300 hover:text-blue-200 ${
    isScrolled ? 'text-white' : 'text-gray-700'
  }`;

  const loginButtonClasses = `font-medium transition-colors duration-300 hover:text-blue-200 ${
    isScrolled ? 'text-white' : 'text-gray-700'
  }`;

  const mobileButtonClasses = `md:hidden transition-colors duration-300 ${
    isScrolled ? 'text-white' : 'text-gray-900'
  }`;

  const mobileBorderClasses = `border-t py-4 ${
    isScrolled ? 'border-white/20' : 'border-gray-200'
  }`;

  const mobileSeparatorClasses = `pt-4 border-t space-y-3 ${
    isScrolled ? 'border-white/20' : 'border-gray-200'
  }`;
    ...
    <header className={headerClasses}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          {/* === MODIFICATION 1 : Boutons d'action dynamiques (Desktop) === */}
          <div className="hidden md:flex items-center space-x-4">
            <SignedOut>
              ...
              <Link href="/sign-up">
                <button className="bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100 transition-all transform hover:scale-105 font-medium border border-white/20">
                  Commencer
                </button>
              </Link>
            </SignedOut>
          ...
          <div className="md:hidden">
            <div className={mobileBorderClasses}>
              <nav className="flex flex-col space-y-4">
                ...
                {/* === MODIFICATION 2 : Boutons d'action dynamiques (Mobile) === */}
                <div className={mobileSeparatorClasses}>
                  <SignedOut>
                    <Link href="/sign-in">
                        <button className={loginButtonClasses} onClick={() => setIsMenuOpen(false)}>
                            Se connecter
                        </button>
                    </Link>
                    <Link href="/sign-up">
                        <button className="w-full bg-white text-blue-600 px-4 py-2 rounded-lg hover:bg-gray-100 transition-all font-medium" onClick={() => setIsMenuOpen(false)}>
                            Commencer
                        </button>
                    </Link>
                  </SignedOut>
                  <SignedIn>
                    ...
                    <div className="flex items-center justify-between pt-2">
                        <span className={navLinkClasses}>Mon Compte</span>
                        <UserButton afterSignOutUrl="/" />
                    </div>
                  </SignedIn>
                </div>
...
// Section Pricing sans pay-per-use
  ...
  const plans = [
    {
      ...
      price: "0€",
      period: "/mois",
      description: "Parfait pour tester notre IA",
      features: [
        "5 documents/mois",
        "IA parsing 85%+ précision",
        "Export CSV basique",
        "Support communauté",
        "Toutes banques françaises"
      ],
      buttonText: "Commencer gratuitement",
      buttonStyle: "bg-gray-100 text-gray-900 hover:bg-gray-200 hover:scale-105",
      cardStyle: "border-gray-200 hover:border-gray-300 hover:shadow-lg",
      popular: false,
    ...
    },
    {
      ...
      price: "49€",
      period: "/mois",
      description: "IA avancée pour PME",
      features: [
        "100 documents/mois",
        "IA parsing 95%+ précision",
        "Auto-catégorisation intelligente",
        "Détection anomalies basique",
        "Exports multiples (CSV, JSON, Excel)",
        "Support email prioritaire",
        "API access basique"
      ],
      buttonText: "Démarrer Smart",
      buttonStyle: "bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:from-blue-700 hover:to-purple-700 hover:scale-105 hover:shadow-xl",
      cardStyle: "border-blue-200 hover:border-blue-400 hover:shadow-2xl hover:-translate-y-2",
      popular: true,
    ...
    },
    {
      ...
      price: "149€",
      period: "/mois",
      description: "IA premium pour cabinets",
      features: [
        "500 documents/mois",
        "IA parsing 97%+ précision",
      ...
      buttonStyle: "bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:from-purple-700 hover:to-pink-700 hover:scale-105 hover:shadow-xl",
      cardStyle: "border-purple-200 hover:border-purple-400 hover:shadow-2xl hover:-translate-y-2",
      popular: false,
    ...
    },
    {
      ...
      price: "399€",
      period: "/mois",
      description: "IA sur-mesure + CSM dédié",
      features: [
        "Documents illimités",
        "IA fine-tuned sur vos données",
        "Custom categorization rules",
        "Utilisateurs illimités",
        "Intégrations ERP custom",
        "CSM dédié + formation",
        "SLA 99.9% + support 24/7",
        "Déploiement on-premise option",
        "Audit compliance inclus"
      ],
      buttonText: "Contacter l'équipe",
      buttonStyle: "bg-gradient-to-r from-yellow-500 to-orange-500 text-white hover:from-yellow-600 hover:to-orange-600 hover:scale-105 hover:shadow-xl",
      cardStyle: "border-yellow-200 hover:border-yellow-400 hover:shadow-2xl hover:-translate-y-2",
      popular: false,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-4">
            ...
            <span className="text-sm font-semibold uppercase tracking-wide">Pricing IA-Powered</span>
          </div>
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            De l'analyse basique à l'IA sur-mesure, trouvez le plan parfait pour automatiser 
            vos relevés bancaires avec une précision révolutionnaire.
          </p>
        </div>

        {/* Grille des cartes pricing */}
              ...
              <div key={plan.name} className={cardClasses}>
                ...
                {plan.popular && (
                  <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                    <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-4 py-2 rounded-full text-sm font-semibold shadow-lg animate-bounce">
                      ⭐ Plus populaire
                    </div>
                  </div>
                )}
                ...
                <div className="text-center mb-4">
                  <div className="flex items-baseline justify-center">
                    <span className="text-5xl font-bold text-gray-900 group-hover:bg-gradient-to-r group-hover:from-blue-600 group-hover:to-purple-600 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
                      {plan.price}
                    </span>
                ...
                {/* Description */}
                <p className="text-gray-600 text-center mb-8 min-h-[48px] flex items-center justify-center">
                  {plan.description}
                </p>
                ...
                <ul className="space-y-4 mb-8">
                  {plan.features.map((feature, featureIndex) => (
                    ...
                    >
                      <div className="flex-shrink-0 w-5 h-5 bg-green-100 rounded-full flex items-center justify-center mt-0.5 group-hover:bg-green-200 transition-colors duration-300">
                ...
                <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-transparent via-white/10 to-transparent -skew-x-12 translate-x-[-100%] group-hover:translate-x-[200%] transition-transform duration-1000"></div>
                </div>
              </div>
            );
          })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div>
                <h3 className="text-xl font-bold">Bank Statement Converter IA</h3>
              </div>
            </div>
            <p className="text-gray-400 mb-6 leading-relaxed">
              La première solution qui utilise l'IA pour convertir vos relevés bancaires 
              avec 99%+ de précision. Automatisez votre comptabilité en quelques clics.
            </p>
  ...
  Politique de confidentialité
        ...
        <div className="border-t border-gray-800 mt-12 pt-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-gray-400">contact@bankstatement-ai.fr</span>
            </div>
        ...
        <div className="border-t border-gray-800 mt-8 pt-8 text-center">
          <p className="text-gray-400">
            © 2024 Bank Statement Converter IA. Tous droits réservés.
          </p>
        </div>
      </div>
    </footer>
...
const BankStatementConverter = () => {
  ...
  // Simulation du processing IA étape par étape
  const simulateAIProcessing = async () => {
    ...
    for (const { step, duration, confidence } of steps) {
    ...
    const mockResults = {
      ...
      transactions: [
        {
          ...
          originalDesc: "VIR SALAIRE ENTREPRISE XYZ REF:SAL240115",
          amount: 3250.00,
          category: "Revenus",
          subcategory: "Salaire",
          confidence: 98.5,
        ...
        },
        {
          id: 2,
          date: "2024-01-16",
          description: "CARTE LECLERC PONTAULT",
          originalDesc: "CB LECLERC PONTAULT 15/01/24",
          amount: -87.45,
        ...
        },
        {
          ...
          originalDesc: "PREL EDF REF:FAC123456",
          amount: -142.30,
          category: "Charges",
        ...
        },
        {
          ...
          originalDesc: "VIR EXTERNE BENEFICIAIRE INCONNU",
          amount: -15000.00,
          category: "Virements",
          subcategory: "Externe",
          confidence: 89.1,
        ...
        },
        {
          id: 5,
          date: "2024-01-19",
          description: "REMBOURSEMENT ASSURANCE SANTE",
          originalDesc: "VIR CPAM REMB SOINS",
          amount: 67.80,
      ...
      summary: {
        totalTransactions: 5,
        totalDebits: -15229.75,
        totalCredits: 3317.80,
        netFlow: -11911.95,
        avgConfidence: 95.4,
        anomaliesDetected: 1
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      {/* Main content avec padding responsive pour compenser la navbar fixe */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              {processing && (
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                  <div className="flex items-center space-x-3 mb-4">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900">Processing IA en cours...</h3>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      ...
                      <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-xl border border-blue-200">
                        <div className="text-2xl font-bold text-blue-700">{results.processingTime}s</div>
                        <div className="text-sm text-blue-600">Temps processing</div>
                      </div>
                      <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-xl border border-purple-200">
                        <div className="text-2xl font-bold text-purple-700">€{results.aiCost}</div>
                        <div className="text-sm text-purple-600">Coût IA</div>
                      </div>
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-gray-200">
                            ...
                            <th className="text-center py-3 px-2 text-sm font-semibold text-gray-700">IA Score</th>
                          </tr>
                        </thead>
                        <tbody>
                          {results.transactions.map((transaction: any) => {
                            const rowClasses = `border-b border-gray-100 hover:bg-gray-50 ${
                              transaction.anomalyScore > 5 ? 'bg-red-50 border-red-200' : ''
                            }`;
                            
                            const amountClasses = `py-3 px-2 text-sm font-semibold text-right ${
                              transaction.amount > 0 ? 'text-green-600' : 'text-red-600'
                            }`;

                            const confidenceClasses = `text-sm font-medium ${
                              transaction.confidence > 95 ? 'text-green-600' :
                              transaction.confidence > 90 ? 'text-yellow-600' : 'text-red-600'
                            }`;
                              ...
                              <tr key={transaction.id} className={rowClasses}>
                                ...
                                <td className="py-3 px-2 text-center">
                                  <div className="flex items-center justify-center space-x-1">
                                    <span className={confidenceClasses}>
                                      {transaction.confidence.toFixed(1)}%
                                    </span>
                                    {transaction.anomalyScore > 5 && (
          ...
          <div id="demo" className="mt-12 bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 text-white text-center">
            ...
            <p className="text-blue-100 max-w-3xl mx-auto">
               pipeline IA complet : OCR → Détection banque → Parsing GPT-4 → Catégorisation → Détection anomalies. 
              Dans la version production, intégration réelle OpenAI + Google Cloud Vision.
            </p>
