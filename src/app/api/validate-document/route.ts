...
async function extractTransactionsWithAI(
  ...
  console.log(
    '[VALIDATE_DOCUMENT] Starting AI-powered transaction extraction...'
  );
  ...
  if (!extractedText || extractedText.length < 50) {
    console.log(
      '[VALIDATE_DOCUMENT] Insufficient text, falling back to basic parsing...'
    );
...
EXEMPLE de format attendu :
{
  "transactions": [
    ...
    console.log(
      '[VALIDATE_DOCUMENT] AI transaction extraction response received'
    );
    ...
    if (aiResponse) {
      ...
      console.log(
        '[VALIDATE_DOCUMENT] AI extracted',
      ...
      console.log(
        '[VALIDATE_DOCUMENT] Formatted',
        formattedTransactions.length,
        'transactions for application'
      );
      console.log(
        '[VALIDATE_DOCUMENT] Sample transactions:',
        formattedTransactions.slice(0, 3)
      );
    ...
    console.error(
      '[VALIDATE_DOCUMENT] AI transaction extraction failed:',
      error
    );
  }
  ...
  console.log(
    '[VALIDATE_DOCUMENT] Falling back to basic transaction generation...'
  );
  return generateBasicTransactions();
}
...
function generateBasicTransactions(): TransactionData[] {
  ...
  console.log(
    '[VALIDATE_DOCUMENT] Generated basic transactions:',
...
async function saveDocumentAndTransactions(
  ...
  try {
    ...
    if (!user) {
      console.log(
        '[VALIDATE_DOCUMENT] No authenticated user, skipping database save'
      );
      return null;
    }

    console.log(
      '[VALIDATE_DOCUMENT] Saving document and transactions to database for user:',
      user.id
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file.type === 'application/pdf') {
      console.log(
        '[VALIDATE_DOCUMENT] PDF detected - processing with integrated solution...'
      );
      ...
      try {
        ...
        console.log('[VALIDATE_DOCUMENT] Calling extractPdfContent...');
        const pdfProcessingResult = await extractPdfContent(pdfBuffer);
        console.log('[VALIDATE_DOCUMENT] extractPdfContent completed');
        console.log(
          '[VALIDATE_DOCUMENT] Processing result:',
          JSON.stringify({
            ...
            containsFinancialData: pdfProcessingResult.contains_financial_data,
            bankKeywordsCount: pdfProcessingResult.bank_keywords_found?.length || 0,
          })
        );

        if (pdfProcessingResult.success && pdfProcessingResult.extracted_text) {
          const extractedText = pdfProcessingResult.extracted_text;
          const base64Image: string | null = null; // extractPdfContent ne retourne pas d'image

          console.log(
            '[VALIDATE_DOCUMENT] Analysis available:',
...
Réponds UNIQUEMENT avec un JSON valide:
{
  ...
  "analysisMethod": ${base64Image ? '"hybrid_text_and_vision"' : '"text_analysis_integrated"'}
          ...
          // Note: pas d'image disponible dans cette version - mode texte seulement
          console.log('[VALIDATE_DOCUMENT] Using text-only analysis');
          
          // Modifier le prompt pour le mode texte seulement
          messageContent[0].text = messageContent[0].text
            .replace(
          ...
          if (aiResponse) {
            try {
              ...
              console.log(
                '[VALIDATE_DOCUMENT] Bank detected:',
                analysis.bankName
              );

              if (analysis.isValidDocument === false) {
                console.log(
                  '[VALIDATE_DOCUMENT] Document rejected by AI analysis:',
              ...
              console.log(
                '[VALIDATE_DOCUMENT] - From AI analysis:',
                analysis.bankName
              );
              ...
              console.log(
                '[VALIDATE_DOCUMENT] Generated transactions count:',
              ...
              console.error(
                '[VALIDATE_DOCUMENT] Failed to parse AI response:',
                parseError
              );
              return NextResponse.json(
          ...
          console.log(
            '[VALIDATE_DOCUMENT] PDF processing failed, no text extracted'
          );
          console.log(
            '[VALIDATE_DOCUMENT] Attempting fallback: basic filename analysis...'
          );

          // FALLBACK ULTIME: analyse du nom de fichier si tout échoue
          const fileName = file.name.toLowerCase();
          const basicBankKeywords = [
            'bnp',
            'paribas',
            'credit',
            'agricole',
          ...
          const foundFileKeyword = basicBankKeywords.find(keyword =>
            fileName.includes(keyword)
          );

          if (foundFileKeyword) {
            console.log(
              '[VALIDATE_DOCUMENT] Found bank keyword in filename:',
              foundFileKeyword
            );
  ...
  "analysisMethod": "filename_fallback"
            ...
            try {
              ...
              if (fallbackResponse) {
                ...
                const analysis = JSON.parse(cleanResponse);
                console.log(
                  '[VALIDATE_DOCUMENT] Fallback analysis result:',
                  analysis.isValidDocument
                );

                if (analysis.isValidDocument !== false) {
                  console.log(
                    '[VALIDATE_DOCUMENT] Document accepted via fallback analysis'
                  );

                  const fallbackTransactions = await extractTransactionsWithAI(
                    fallbackText,
                    undefined
                  );
                  console.log(
                    '[VALIDATE_DOCUMENT] Fallback transactions generated:',
              ...
              console.error(
                '[VALIDATE_DOCUMENT] Fallback analysis failed:',
                fallbackError
              );
            }
          }
        ...
        console.error(
          '[VALIDATE_DOCUMENT] PDF processing error stack:',
...
Réponds UNIQUEMENT avec un JSON valide:
{
  ...
  "analysisMethod": "gpt4_vision_analysis"
    ...
    console.log(
      '[VALIDATE_DOCUMENT] No valid processing completed, rejecting by default'
    );
    return NextResponse.json(
      {
        error: 'DOCUMENT_REJECTED',
        message:
          "Document non valide: Impossible d'analyser le document. Veuillez vous assurer que le fichier est un relevé bancaire ou une facture valide.",
        documentType: 'autre',
      },
      { status: 400 }
    );
