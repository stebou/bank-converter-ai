...
export async function POST(req: NextRequest) {
  try {
    ...
    if (documents.length !== documentIds.length) {
      return NextResponse.json({ error: 'Certains documents n\'appartiennent pas à l\'utilisateur' }, { status: 403 });
    }
    ...
    return NextResponse.json({ error: 'Erreur lors de la suppression en lot' }, { status: 500 });
  }
}