import { sireneService, SireneService } from '@/lib/sirene/service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `🔍 Interrogation unitaire SIREN: ${siren}${date ? ` à la date ${date}` : ''}`
    );

    try {
      const sireneResponse = await sireneService.getUniteLegale(siren, date);

      // Extraire les données courantes pour faciliter l'usage
      const currentData = SireneService.getCurrentPeriodData(
        sireneResponse.uniteLegale
      );
      ...
      return NextResponse.json({
        ...
        historicalPeriods:
          sireneResponse.uniteLegale?.periodesUniteLegale?.length || 0,
        source: 'sirene',
      });
