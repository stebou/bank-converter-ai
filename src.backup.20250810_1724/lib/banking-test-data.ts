...
export async function seedTestBankingData(clerkUserId: string) {
  ...
  try {
    ...
    console.log(
      `[BANKING_TEST] Found user ${userId} for Clerk ID ${clerkUserId}`
    );
    ...
    const transactionsData = mockBankTransactions.map(trans => {
      ...
      const newAccountId = createdAccounts.find(
        acc =>
          acc.bridgeAccountId ===
          mockBankAccounts.find(mock => mock.id === oldAccountId)
            ?.bridgeAccountId
      )?.id;
    ...
    console.log(
      `[BANKING_TEST] Created ${createdAccounts.length} accounts and ${transactionsData.length} transactions for user ${userId} (Clerk: ${clerkUserId})`
    );
