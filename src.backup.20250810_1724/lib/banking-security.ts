...
const ENCRYPTION_KEY =
  process.env.BANKING_ENCRYPTION_KEY || 'default-key-change-in-production';
...
export function maskSensitiveFields(data: any): any {
  ...
  if (masked.iban) {
    masked.iban =
      masked.iban.substring(0, 4) +
      '****' +
      masked.iban.substring(masked.iban.length - 4);
  }

  if (masked.accountNumber) {
    masked.accountNumber =
      '****' + masked.accountNumber.substring(masked.accountNumber.length - 4);
  }
  ...
  if (masked.description && masked.description.length > 6) {
    ...
    masked.description =
      desc.substring(0, 2) + '***' + desc.substring(desc.length - 2);
  }
