...
export class AnalysisStorage {
  ...
  static async saveStockAnalysis(analysisData: any): Promise<StockAnalysis> {
    try {
      const analysis: StockAnalysis = {
        ...
        duration: analysisData.execution_time_ms
          ? analysisData.execution_time_ms / 1000
          : 5.0,
   ...
   */
  static async saveFinancialAnalysis(
    analysisData: any
  ...
  private static generateStockSummary(data: any): string {
    const productsCount =
      data.products_analyzed || data.raw_data?.sales_history?.length || 0;
  ...
  private static extractStockInsights(data: any): string[] {
    ...
    if (data.anomalies?.length > 0) {
      insights.push(
        `Détection de ${data.anomalies.length} anomalies critiques nécessitant action immédiate`
      );
    }
    ...
    if (data.optimization_results?.savings_potential) {
      const savings =
        Math.round(data.optimization_results.savings_potential * 100) / 100;
      insights.push(
        `Optimisation EOQ générant ${savings}% d'économies potentielles`
      );
    } else {
    ...
    if (data.recommendations?.length > 0) {
      insights.push(
        `${data.recommendations.length} recommandations d'optimisation générées`
      );
    }
  ...
  private static extractFinancialInsights(data: any): string[] {
    ...
    if (transactions > 0) {
      insights.push(
        `Détection automatique de ${transactions} transactions avec 98.5% précision`
      );
    }

    if (data.suspicious_transactions?.length > 0) {
      insights.push(
        `Identification de ${data.suspicious_transactions.length} transactions nécessitant vérification`
      );
    } else if (transactions > 20) {
      const suspicious = Math.ceil(transactions * 0.05); // 5% estimé
      insights.push(
        `Identification de ${suspicious} transactions nécessitant vérification`
      );
    }
    ...
    if (amount > 0) {
      ...
      insights.push(
        `Optimisation flux de trésorerie: économies potentielles ${savings.toLocaleString('fr-FR')}€`
      );
    }

    if (data.trend_analysis) {
      insights.push(
        'Analyse tendances: évolution des charges vs mois précédent'
      );
    } else {
      insights.push(
