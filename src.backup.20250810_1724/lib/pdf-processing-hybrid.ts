...
export async function extractPdfContent(pdfBuffer: Buffer): Promise<{
  ...
  try {
    ...
    const contains_financial_data =
      foundFinancialKeywords.length > 0 || foundBankKeywords.length > 0;

    console.log('[PDF_PROCESSING] Bank keywords found:', foundBankKeywords);
    console.log(
      '[PDF_PROCESSING] Financial data detected:',
...
export function generateTestBankContent(
  bankKeywords: string[],
  fileName: string
): string {
  console.log(
    '[PDF_PROCESSING] Generating test content based on keywords:',
    bankKeywords
  );
  ...
  if (bankKeywords.some(k => k.includes('bnp') || k.includes('paribas'))) {
    ...
    bankKeywords.some(k => k.includes('crédit') && k.includes('agricole'))
  ) {
    mainBank = 'Crédit Agricole';
  } else if (
    bankKeywords.some(k => k.includes('société') || k.includes('générale'))
  ) {
