...
export async function processPdfServerSide(
  pdfBuffer: Buffer,
  filename?: string
): Promise<{
  success: boolean;
  ...
  try {
    ...
    console.log(
      '[PDF_SERVER] Python processing completed:',
    ...
    if (pythonResult.success) {
      ...
      processingMethod =
        pythonResult.metadata?.processing_method || 'python_success';
      detectedBank = pythonResult.metadata?.detected_bank || '';

      console.log(
        '[PDF_SERVER] Text extracted:',
        extractedText.length,
        'chars'
      );
      console.log(
        '[PDF_SERVER] Image converted:',
        imageBase64.length,
        'chars base64'
      );
      ...
      console.error(
        '[PDF_SERVER] Python processing failed:',
        pythonResult.error
      );
  ...
  const foundKeywords = bankingKeywords.filter(keyword =>
  ...
  console.log(
    '[PDF_SERVER] - Has image:',
    hasImage,
    `(${imageBase64.length} chars base64)`
  );
