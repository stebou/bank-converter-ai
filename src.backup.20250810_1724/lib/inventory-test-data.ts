...
export const generateMockMovements = (
  ...
  products.forEach((product, prodIndex) => {
    ...
    for (let i = 0; i < numMovements; i++) {
      const types: Array<'IN' | 'OUT' | 'ADJUSTMENT'> = [
        'IN',
        'OUT',
        'ADJUSTMENT',
      ];
...
export const generateMockOdooProducts = (): MockOdooProduct[] => {
  const odooProducts = [
    ...
    {
      name: 'Article Odoo Premium',
      sku: 'ODOO-PREM-003',
      category: 'Premium',
      price: 299,
    },
    {
      name: 'Service Odoo Standard',
      sku: 'ODOO-STD-004',
      category: 'Services',
      price: 89,
    },
    {
      name: 'Composant Odoo Tech',
      sku: 'ODOO-TECH-005',
      category: 'Technique',
      price: 45,
    },
  ...
  async getProducts(): Promise<{
    ...
    this.products = this.products.map(product => ({
      ...
      currentStock: Math.max(
        0,
        product.currentStock + Math.floor(Math.random() * 10) - 5
      ), // Variation aléatoire
    }));
...
export const generateInventoryStats = (
  ...
  return {
    ...
    turnoverRate:
      weeklyOutgoing > 0
        ? ((weeklyOutgoing / totalProducts) * 100).toFixed(1)
        : '0',
  };
};
