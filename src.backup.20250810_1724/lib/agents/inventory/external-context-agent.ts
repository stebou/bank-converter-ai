...
export class ExternalContextAgent extends BaseAgent {
  ...
  private batchCache: Map<
    string,
    { insights: MarketInsight[]; timestamp: Date }
  > = new Map();
  ...
  private async batchOpenAIAnalysis(
    ...
    try {
      ...
      if (
        cached &&
        Date.now() - cached.timestamp.getTime() < this.BATCH_CACHE_DURATION_MS
      ) {
      ...
      this.log('info', 'Starting batched OpenAI analysis', {
        ...
        totalWebResults: searches.reduce(
          (sum, search) => sum + search.webResults.length,
          0
        ),
        cacheKey: cacheKey.substring(0, 50) + '...',
      });
      ...
      const batchPrompt = searches
        .map((search, index) => {
          const webContent = search.webResults
            .map(
              result =>
...
Format attendu:
{
        ...
        for (const insight of batchInsight.insights || []) {
          allInsights.push({
            ...
            confidence_score: Math.min(
              1,
              Math.max(0, insight.confidence_score || 0.7)
            ),
            ...
            predicted_impact: {
              demand_change_percentage:
                insight.predicted_impact?.demand_change_percentage || 0,
            ...
            keywords: [
              'market_trends',
              'ai_analysis',
              input.company_profile.industry,
            ],
          });
        }
      }
  ...
  async execute(
    ...
    try {
      ...
      if (!process.env.OPENAI_API_KEY) {
        ...
        throw new Error(
          'OPENAI_API_KEY is required for external context analysis'
        );
      }

      this.log(
        'info',
        'OpenAI integration available, proceeding with market trends analysis'
      );
      ...
      this.log('info', 'External context analysis completed', {
        ...
        competitorActions: competitorAnalysis.reduce(
          (sum, comp) => sum + comp.actions_detected.length,
          0
        ),
  ...
  private async searchMarketTrends(
    input: ExternalContextInput
  ): Promise<MarketInsight[]> {
    ...
    const categoryQueries = input.company_profile.product_categories.map(
      category => `${category} demand forecast ${industry}`
    );
    ...
    this.log('info', 'Preparing batch web searches', {
  ...
  private async analyzeCompetitor(
    ...
    if (Math.random() > 0.6) {
      ...
      actions.push({
        ...
        affected_categories: input.company_profile.product_categories.slice(
          0,
          2
        ),
    ...
    if (Math.random() > 0.8) {
      // 20% chance de lancement produit
      actions.push({
    ...
    return {
      ...
      market_position: {
        ...
        distribution_channels: ['online', 'retail', 'B2B'].filter(
          () => Math.random() > 0.5
        ),
      },
      recent_activities: {
      ...
      threat_assessment:
        actions.length > 1 ? 'HIGH' : actions.length > 0 ? 'MEDIUM' : 'LOW',
    };
  }
    ...
    const economicEvents = [
      {
        ...
        description:
          'Les prévisions économiques anticipent une inflation de 2.8% au Q4',
        impact: {
          demand_impact: -0.1,
          price_impact: 0.2,
          supply_chain_impact: -0.05,
        },
      },
      {
        title: "Taux d'intérêt stable maintenu",
        description:
          'La banque centrale maintient les taux directeurs inchangés',
        impact: {
          demand_impact: 0.05,
          price_impact: 0,
          supply_chain_impact: 0.02,
        },
      },
    ];
  ...
  private async analyzeMarketSentiment(
    ...
    try {
      ...
      const sentimentData = await this.executeWebSearch(
        sentimentQuery,
        input.company_profile.industry
      );
      ...
      const webContent = sentimentData
        .map(
          result =>
            `Source: ${result.source}\nTitle: ${result.title}\nContent: ${result.content}\n---`
        )
...
Analyse le sentiment global et par catégorie.`;
      ...
      for (const competitor of input.company_profile.competitors) {
        ...
        competitorSentiments[competitor] =
          parsedResponse.overall_sentiment + (Math.random() - 0.5) * 0.4;
      }

      const sentimentAnalysis: SentimentAnalysis = {
        overall_sentiment: Math.min(
          1,
          Math.max(-1, parsedResponse.overall_sentiment || 0)
        ),
        sentiment_trend: parsedResponse.sentiment_trend || 'STABLE',
        key_topics: parsedResponse.key_topics || [],
        brand_perception: {
          company:
            parsedResponse.overall_sentiment + (Math.random() - 0.5) * 0.2,
          competitors: competitorSentiments,
        },
        consumer_confidence: {
          current_level: Math.min(
            100,
            Math.max(0, parsedResponse.consumer_confidence?.current_level || 75)
          ),
          three_month_outlook: Math.min(
            100,
            Math.max(
              0,
              parsedResponse.consumer_confidence?.three_month_outlook || 75
            )
          ),
          industry_specific: Math.min(
            100,
            Math.max(
              0,
              parsedResponse.consumer_confidence?.industry_specific || 70
            )
          ),
        },
      };
  ...
  private fallbackSentimentAnalysis(
    ...
    const keyTopics = input.company_profile.product_categories.map(
      category => ({
        topic: category,
  ...
  private async performRealWebSearch(
    ...
    try {
      this.log('info', 'Performing real web search', {
        query,
        type,
        industry: input.company_profile.industry,
      });
      ...
      const webSearchResults = await this.executeWebSearch(
        query,
        input.company_profile.industry
      );
      ...
      const analyzedInsights = await this.analyzeWebResultsWithOpenAI(
        webSearchResults,
        query,
        type,
        input
  ...
  private async executeWebSearch(
    ...
    try {
      ...
      this.log('info', 'Executing real web search with SerpAPI', {
        optimizedQuery,
      });
      ...
      if (!process.env.SERPAPI_API_KEY) {
        this.log(
          'warn',
          'SERPAPI_API_KEY not found, falling back to OpenAI simulation'
        );
      ...
      this.log(
        'error',
        'SerpAPI search failed, falling back to OpenAI simulation',
        {
          error: error instanceof Error ? error.message : error,
        }
      );
  ...
  private async generateEnhancedWebResults(
    query: string,
    industry: string
  ): Promise<any[]> {
    try {
      ...
      if (
        ...
        throw new Error(
          `Invalid thread ID received: ${threadId}. Expected ID starting with 'thread_'`
        );
      }
...
Fournis des insights exploitables pour la gestion des stocks et la prévision de la demande.`,
      ...
      const run = await this.openai.beta.threads.runs.create(threadId, {
        ...
        instructions:
          'Analyse les données demandées et fournis des insights structurés pour la gestion des stocks.',
      });
      ...
      if (!threadId || typeof threadId !== 'string') {
        throw new Error(
          `threadId is undefined or invalid before run retrieval: ${threadId}`
        );
      }
      ...
      while (
        ...
        if (!run?.id || !threadId) {
          throw new Error(
            `IDs undefined during retry - runId: ${run?.id}, threadId: ${threadId}`
          );
        }
      ...
      if (!threadId || typeof threadId !== 'string') {
        throw new Error(
          `threadId is undefined or invalid before message retrieval: ${threadId}`
        );
      }
  ...
  private convertOpenAIToWebResults(
    ...
    const premiumSources = [
      ...
      },
      {
        ...
        domain: 'bcg.com',
        reliability: 0.94,
        focus: 'Innovation insights',
      },
    ];

    // Diviser le contenu OpenAI en segments pour chaque source
    const contentSegments = marketContent
      .split('\n\n')
  ...
  private parseWebSearchResults(
    ...
    premiumSources.forEach((source, index) => {
      const relevantSegment =
        segments[index] || webContent.substring(index * 200, (index + 1) * 200);

      results.push({
        title: `${source.focus} for ${industry}: Latest insights from ${source.name}`,
        content:
          relevantSegment.length > 50
            ? relevantSegment
  ...
  private generateFallbackWebResults(query: string, industry: string): any[] {
    ...
    results.push({
      ...
      date: new Date(
        currentDate.getTime() - Math.random() * 5 * 24 * 60 * 60 * 1000
      ).toISOString(),
    ...
    results.push({
      ...
      date: new Date(
        currentDate.getTime() - Math.random() * 8 * 24 * 60 * 60 * 1000
      ).toISOString(),
    ...
    results.push({
      ...
      date: new Date(
        currentDate.getTime() - Math.random() * 12 * 24 * 60 * 60 * 1000
      ).toISOString(),
      source: 'PwC Global',
      reliability: 0.9,
    ...
    results.push({
      ...
      date: new Date(
        currentDate.getTime() - Math.random() * 6 * 24 * 60 * 60 * 1000
      ).toISOString(),
    ...
    this.log('info', 'Generated premium web results', {
      ...
      avgReliability:
        results.reduce((sum, r) => sum + r.reliability, 0) / results.length,
    });
...
Analyse ces données et fournis des insights exploitables pour optimiser la gestion des stocks de cette entreprise.`;
  ...
  private async fallbackToSimulation(
    ...
    for (let i = 0; i < numResults; i++) {
      ...
      insights.push({
        ...
        affected_products: this.selectAffectedProducts(
          input.company_profile.product_categories
        ),
        predicted_impact: {
          demand_change_percentage: this.calculateDemandImpact(
            impactScore,
            type
          ),
          direction:
  ...
  private initializeWebSources(): void {
    ...
    this.log('info', 'Initialized premium web sources', {
      ...
      avgReliability:
        sources.reduce((sum, s) => sum + s.reliability_score, 0) /
        sources.length,
  ...
  private initializeIndustryKeywords(): void {
    const keywords = new Map([
      [
        'technology',
        ['innovation', 'digital', 'AI', 'cloud', 'cybersecurity', 'blockchain'],
      ],
      [
        'retail',
        ['consumer', 'shopping', 'e-commerce', 'omnichannel', 'experience'],
      ],
  ...
  private determineTimeRelevance(
    impactScore: number
  ): MarketInsight['time_relevance'] {
  ...
  private detectSeasonalEvents(input: ExternalContextInput): MarketEvent[] {
    ...
    const industryEvents =
      seasonalEvents[
        input.company_profile.industry as keyof typeof seasonalEvents
      ] || [];
    ...
    for (const event of industryEvents) {
      ...
      if (
        ...
        eventDate.getTime() < now.getTime() + 90 * 24 * 60 * 60 * 1000
      ) {
        events.push({
  ...
  private rankAndFilterInsights(
    ...
    return insights
      .sort(
        (a, b) =>
          b.impact_score * b.confidence_score -
          a.impact_score * a.confidence_score
      )
      .slice(0, maxResults);
  }
    ...
    for (const event of events) {
      allInsights.push({
        ...
        predicted_impact: {
          ...
          direction:
            event.predicted_effects.demand_impact > 0 ? 'INCREASE' : 'DECREASE',
          duration_days: 30,
        },
        discovered_at: new Date(),
  ...
  private async generateContextualRecommendations(
    ...
    for (const insight of criticalInsights) {
      if (insight.predicted_impact.direction === 'INCREASE') {
        recommendations.push(
          `Préparer hausse demande ${insight.predicted_impact.demand_change_percentage}% pour ${insight.affected_products.join(', ')}`
        );
      } else if (insight.predicted_impact.direction === 'DECREASE') {
        recommendations.push(
          `Anticiper baisse demande ${Math.abs(insight.predicted_impact.demand_change_percentage)}% pour ${insight.affected_products.join(', ')}`
        );
      }
    }
    ...
    if (trendInsights.length > 3) {
      recommendations.push(
        'Plusieurs tendances marché détectées - révision stratégique recommandée'
      );
    }
    ...
    for (const insight of insights) {
      if (insight.impact_score > 0.5) {
        for (const productId of insight.affected_products) {
          const adjustment =
            insight.predicted_impact.demand_change_percentage / 100;
  ...
  private eventMagnitudeToScore(
    magnitude: MarketEvent['impact_magnitude']
  ): number {
    const scores = { LOW: 0.3, MEDIUM: 0.6, HIGH: 0.8, CRITICAL: 0.95 };
    return scores[magnitude];
  }

  private eventToTimeRelevance(
    startDate: Date
  ): MarketInsight['time_relevance'] {
    const daysUntil =
  ...
  private calculateGlobalMarketFactor(insights: MarketInsight[]): number {
    ...
    const weightedImpact =
      insights.reduce(
        (sum, insight) => sum + insight.impact_score * insight.confidence_score,
        0
      ) / insights.length;
  ...
  private calculateAdjustmentConfidence(insights: MarketInsight[]): number {
    ...
    return (
      insights.reduce((sum, insight) => sum + insight.confidence_score, 0) /
      insights.length
    );
  }
  ...
  private calculateContextConfidence(insights: MarketInsight[]): number {
    ...
    const avgConfidence =
      insights.reduce((sum, insight) => sum + insight.confidence_score, 0) /
      insights.length;
    const highImpactRatio =
      insights.filter(i => i.impact_score > 0.6).length / insights.length;
  ...
  private generateIntelligenceSummary(insights: MarketInsight[]): string {
    ...
    const highImpactInsights = insights.filter(
      i => i.impact_score > 0.6
    ).length;
    const immediateInsights = insights.filter(
      i => i.time_relevance === 'IMMEDIATE'
    ).length;
  ...
  private calculateNextMonitoringSchedule(input: ExternalContextInput): Date {
    ...
    const hoursUntilNext = input.search_configuration
