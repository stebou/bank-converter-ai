...
export class AnomalyDetectionAgent extends BaseAgent {
  ...
  async execute(
    ...
    try {
      ...
      const realTimeAlerts =
        await this.generateRealTimeAlerts(prioritizedAnomalies);
      ...
      this.log('info', 'Anomaly detection completed', {
        ...
        criticalAnomalies: prioritizedAnomalies.filter(
      ...
      return {
        ...
        success: true,
        confidence_score:
          this.calculateDetectionConfidence(prioritizedAnomalies),
        output: {
          ...
          detection_summary:
            this.generateDetectionSummary(prioritizedAnomalies),
          recommendations:
            this.generateSystemRecommendations(prioritizedAnomalies),
        },
        metrics: {
          accuracy: this.calculateAccuracy(prioritizedAnomalies),
          processing_speed:
            (prioritizedAnomalies.length / executionTime) * 1000,
  ...
  private async updateStatisticalBaselines(
    input: AnomalyDetectionInput
  ): Promise<void> {
    const productIds = [
      ...new Set(input.sales_history.map(sale => sale.product_id)),
    ];

    for (const productId of productIds) {
      const productSales = input.sales_history.filter(
        sale => sale.product_id === productId
      );
      const baseline = this.calculateStatisticalBaseline(
        productId,
        productSales,
        input.detection_parameters
      );
      this.baselines.set(productId, baseline);
    }

    this.log('info', 'Statistical baselines updated', {
  ...
  private async detectDemandAnomalies(
    input: AnomalyDetectionInput
    ...
    for (const [productId, baseline] of this.baselines.entries()) {
      const recentSales = input.sales_history.filter(
        sale =>
      ...
      const recentDemand = recentSales.reduce(
      ...
      const deviationScore =
        Math.abs(recentDemand - adjustedExpectedDemand) /
        (baseline.std_dev * Math.sqrt(recentWindow));
      ...
      if (deviationScore > threshold) {
        ...
        anomalies.push({
          ...
          impact_assessment: this.assessImpact(
            ...
            isSpike ? 'spike' : 'drop',
            recentDemand,
            adjustedExpectedDemand
          ),
          root_cause_analysis: this.analyzeRootCause(
            productId,
            isSpike ? 'spike' : 'drop',
            input
          ),
          recommended_actions: this.generateActionRecommendations(
            productId,
            isSpike ? 'spike' : 'drop',
            deviationScore
          ),
  ...
  private async detectForecastDeviations(
    input: AnomalyDetectionInput
  ): Promise<AnomalyResult[]> {
    const anomalies: AnomalyResult[] = [];
    ...
    for (const forecast of input.forecasts) {
      ...
      const forecastError =
        Math.abs(actualSales - forecast.predicted_demand) /
        Math.max(forecast.predicted_demand, 1);
      const errorThreshold = 1 - forecast.accuracy_score; // Seuil basé sur la confiance du modèle

      if (forecastError > errorThreshold * 2) {
        // Erreur significativement plus élevée que prévu
        anomalies.push({
          ...
          impact_assessment: this.assessForecastImpact(
            forecast.product_id,
            forecastError
          ),
          root_cause_analysis: [
            ...
            'Facteur externe non modélisé',
          ],
          recommended_actions: [
            'Réviser modèle de prévision',
            'Analyser facteurs externes',
            'Ajuster paramètres',
          ],
        });
      }
  ...
  private async detectInventoryAnomalies(
    ...
    for (const inventory of input.current_inventory) {
      ...
      const daysOfStock =
        inventory.available_quantity / Math.max(dailyDemandAvg, 0.1);

      // Détection de surstockage
      if (daysOfStock > 90) {
        // Plus de 3 mois de stock
        anomalies.push({
          ...
          impact_assessment: this.assessInventoryImpact(
  ...
  private async detectSeasonalAnomalies(
    ...
    for (const pattern of input.demand_patterns) {
      ...
      const expectedSeasonalMultiplier =
        baseline.seasonal_adjustments[currentMonth] || 1;
      const recentSales = input.sales_history.filter(
        sale =>
      ...
      const recentAvgDaily =
        recentSales.reduce((sum, sale) => sum + sale.quantity_sold, 0) / 30;
      const expectedSeasonalDemand = baseline.mean * expectedSeasonalMultiplier;
      const seasonalDeviation =
        Math.abs(recentAvgDaily - expectedSeasonalDemand) /
        expectedSeasonalDemand;

      if (seasonalDeviation > 0.3) {
        // 30% d'écart par rapport au pattern saisonnier
        anomalies.push({
          ...
          impact_assessment: this.assessSeasonalImpact(
            pattern.product_id,
            seasonalDeviation
          ),
          root_cause_analysis: [
            'Changement tendance marché',
            'Événement exceptionnel',
            'Evolution comportement client',
          ],
          recommended_actions: [
            'Réviser modèle saisonnier',
  ...
  private async detectSupplyChainAnomalies(
    input: AnomalyDetectionInput
  ): Promise<AnomalyResult[]> {
    const anomalies: AnomalyResult[] = [];
  ...
  private aggregateDailyDemands(sales: SalesRecord[]): {
    [date: string]: number;
  } {
  ...
  private calculateSeasonalAdjustments(sales: SalesRecord[]): {
    [month: number]: number;
  } {
    ...
    for (let month = 0; month < 12; month++) {
      monthlyAverages[month] =
        (monthlyTotals[month] || 0) / (monthlyCounts[month] || 1);
    }

    const overallAverage =
      Object.values(monthlyAverages).reduce((sum, avg) => sum + avg, 0) / 12;
  ...
  private calculateSeverityFromError(
    errorRate: number
  ): AnomalyResult['severity'] {
    ...
    for (const anomaly of anomalies) {
      if (anomaly.severity === 'CRITICAL' || anomaly.severity === 'HIGH') {
        alerts.push({
          ...
          recommended_action:
            anomaly.recommended_actions[0] || 'Investigation requise',
        });
      }
    }
  ...
  private analyzeTrends(anomalies: AnomalyResult[]): any {
    ...
    for (const anomaly of anomalies) {
      trendsByType[anomaly.anomaly_type] =
        (trendsByType[anomaly.anomaly_type] || 0) + 1;
      trendsBySeverity[anomaly.severity] =
        (trendsBySeverity[anomaly.severity] || 0) + 1;
    }
    ...
    return {
      ...
      most_frequent_type:
        Object.entries(trendsByType).sort(([, a], [, b]) => b - a)[0]?.[0] ||
        'NONE',
  ...
  private assessImpact(
  ...
  ): AnomalyResult['impact_assessment'] {
    ...
    return {
      ...
      operational_impact:
        type === 'spike' ? 'Risque de rupture' : 'Surstockage potentiel',
      customer_impact:
        type === 'spike' ? 'Satisfaction accrue' : 'Risque de mécontentement',
    };
  }

  private assessForecastImpact(
    productId: string,
    errorRate: number
  ): AnomalyResult['impact_assessment'] {
    return {
  ...
  private assessSupplyChainImpact(
    productId: string,
    alertType: string
  ): AnomalyResult['impact_assessment'] {
    return {
  ...
  private generateSystemRecommendations(anomalies: AnomalyResult[]): string[] {
    ...
    if (criticalCount > 0) {
      recommendations.push(
        `${criticalCount} anomalies critiques détectées - intervention immédiate requise`
      );
    }

    const demandAnomalies = anomalies.filter(a =>
      a.anomaly_type.includes('DEMAND')
    ).length;
    if (demandAnomalies > anomalies.length * 0.5) {
      recommendations.push(
        'Forte volatilité de la demande - réviser stratégie de prévision'
      );
    }

    const forecastDeviations = anomalies.filter(
      a => a.anomaly_type === 'FORECAST_DEVIATION'
    ).length;
    if (forecastDeviations > 3) {
      recommendations.push(
        'Modèles de prévision défaillants - recalibration nécessaire'
      );
    }
  ...
  private calculateDetectionConfidence(anomalies: AnomalyResult[]): number {
    ...
    const avgConfidence =
      anomalies.reduce((sum, anomaly) => sum + anomaly.confidence_score, 0) /
      anomalies.length;
  ...
  private calculateAccuracy(anomalies: AnomalyResult[]): number {
    ...
    const highConfidenceAnomalies = anomalies.filter(
      a => a.confidence_score > 0.7
    ).length;
    return anomalies.length > 0
      ? highConfidenceAnomalies / anomalies.length
      : 0.9;
  }

  private calculateAnomalyTrend(): string {
    const recentAnomalies = this.anomalyHistory.filter(
      a => a.detected_at > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
    ).length;

    const previousWeekAnomalies = this.anomalyHistory.filter(
      a =>
  ...
  private generateDetectionSummary(anomalies: AnomalyResult[]): string {
    ...
    return (
      `Détection de ${total} anomalies: ${critical} critiques, ${high} importantes. ` +
  ...
  private getMostFrequentTypes(anomalies: AnomalyResult[]): string[] {
    ...
    for (const anomaly of anomalies) {
      typeCounts[anomaly.anomaly_type] =
        (typeCounts[anomaly.anomaly_type] || 0) + 1;
    }
