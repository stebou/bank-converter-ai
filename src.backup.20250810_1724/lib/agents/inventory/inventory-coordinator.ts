...
export class MasterCoordinatorAgent extends BaseAgent {
  ...
  async execute(
    ...
    try {
      ...
      const workflowResult = await this.executeDailyAnalysisWorkflow(
        input,
        state
      );

      // Synthèse finale et génération des recommandations
      const finalRecommendations = await this.synthesizeInsights(
        workflowResult.final_state
      );

      // Mise à jour des métriques temps réel
      const realTimeMetrics = this.updateRealTimeMetrics(
        workflowResult.final_state
      );

      // Extraction de l'intelligence marché depuis le workflow
      const marketIntelligence =
        workflowResult.performance_summary.agents_performance[
          'external-context'
        ]?.output || null;
      ...
      return {
        ...
        metrics: {
          ...
          processing_speed:
            (workflowResult.agents_executed.length / executionTime) * 1000,
  ...
  async executeDailyAnalysisWorkflow(
    input: any,
    initialState: StockAnalysisState
    ...
    try {
      ...
      if (
        forecastingAgent &&
        currentState.processed_insights?.demand_patterns?.length > 0
      ) {
        const forecastingResult = await forecastingAgent.run(
          {
            historical_data: currentState.raw_data?.sales_history || [],
            demand_patterns:
              currentState.processed_insights?.demand_patterns || [],
        ...
        if (forecastingResult.success && forecastingResult.output) {
          ...
          currentState.forecasts = {
            short_term: forecastingResult.output.forecasts.filter(
              (f: any) => this.daysBetween(new Date(), f.forecast_date) <= 7
            ),
            medium_term: forecastingResult.output.forecasts.filter(
              (f: any) =>
                this.daysBetween(new Date(), f.forecast_date) > 7 &&
                this.daysBetween(new Date(), f.forecast_date) <= 30
            ),
            long_term: forecastingResult.output.forecasts.filter(
              (f: any) => this.daysBetween(new Date(), f.forecast_date) > 30
            ),
            confidence_intervals: this.extractConfidenceIntervals(
              forecastingResult.output.forecasts
            ),
          };
        }
      }
      ...
      if (
        ...
        if (optimizationResult.success && optimizationResult.output) {
          currentState.optimization_results = {
            ...
            order_quantities:
              optimizationResult.output.eoq_results?.map((eoq: any) => ({
                ...
                total_cost_per_cycle:
                  eoq.total_annual_cost / (365 / eoq.cycle_length_days),
              })) || [],
          };
        }
      }
      ...
      if (anomalyDetectionAgent) {
        const anomalyResult = await anomalyDetectionAgent.run(
          {
            sales_history: currentState.raw_data?.sales_history || [],
            forecasts: currentState.forecasts.short_term.concat(
              currentState.forecasts.medium_term
            ),
            demand_patterns:
              currentState.processed_insights?.demand_patterns || [],
            current_inventory: currentState.raw_data?.current_inventory || [],
        ...
        try {
          ...
          if (contextResult.success && contextResult.output) {
            ...
            const marketRecommendations =
              contextResult.output.contextual_recommendations || [];
            currentState.recommendations = [
              ...(currentState.recommendations || []),
              ...marketRecommendations.map((rec: string, index: number) => ({
                ...
                action: rec,
                reasoning:
                  'Recommandation basée sur intelligence marché externe',
                expected_impact:
                  'Optimisation stratégique basée sur contexte marché',
            ...
            if (contextResult.output.forecast_adjustments) {
              this.log('info', 'Applying market-based forecast adjustments', {
                adjustments: Object.keys(
                  contextResult.output.forecast_adjustments
                    .product_adjustments || {}
                ).length,
                globalFactor:
                  contextResult.output.forecast_adjustments
          ...
          this.log(
            'warn',
            'External context agent failed, continuing without market intelligence',
            {
              error: error instanceof Error ? error.message : error,
            }
          );
  ...
  async synthesizeInsights(
    ...
    try {
      ...
      this.log('info', 'Insights synthesized', {
        ...
        criticalRecommendations: prioritizedRecommendations.filter(
      ...
      this.log('error', 'Failed to synthesize insights', {
        error: error instanceof Error ? error.message : error,
      });
      return [];
    }
  ...
  private async generateAlerts(state: StockAnalysisState): Promise<Alert[]> {
    ...
    try {
      ...
      this.log('error', 'Failed to generate alerts', {
        error: error instanceof Error ? error.message : error,
      });
      return [];
    }
  ...
  private generateDemandBasedRecommendations(
    state: StockAnalysisState
  ): Recommendation[] {
    const recommendations: Recommendation[] = [];
      ...
      switch (pattern.pattern_type) {
        case 'SEASONAL':
          if (pattern.seasonality_strength > 0.7) {
            recommendations.push({
              ...
              expected_impact:
                'Réduction des ruptures de 25% et optimisation des coûts de stockage',
        ...
        case 'ERRATIC':
          if (pattern.volatility > 0.6) {
            recommendations.push({
              ...
              expected_impact:
                'Amélioration du service client malgré la volatilité',
              confidence_score: pattern.confidence,
              estimated_cost: 500,
            });
          }
        ...
        case 'TRENDING':
          if (pattern.trend_strength > 0.3) {
            const direction =
              pattern.trend_direction === 'UP' ? 'croissante' : 'décroissante';
            recommendations.push({
              ...baseRecommendation,
              type: 'ADJUST',
  ...
  private generateForecastBasedRecommendations(
    state: StockAnalysisState
  ): Recommendation[] {
    const recommendations: Recommendation[] = [];
    ...
    for (const forecast of state.forecasts.short_term) {
      if (forecast.accuracy_score < 0.6) {
        recommendations.push({
          ...
          expected_impact:
            'Amélioration de la précision des prévisions futures',
      ...
      if (forecast.predicted_demand > forecast.confidence_95[1] * 0.8) {
        recommendations.push({
          ...
          deadline: new Date(
            forecast.forecast_date.getTime() - 7 * 24 * 60 * 60 * 1000
          ),
    ...
    for (const segment of state.processed_insights.product_segments) {
      ...
      if (segment.strategic_importance === 'CRITICAL') {
        recommendations.push({
          ...
          expected_impact:
            'Garantir disponibilité continue du produit stratégique',
  ...
  private prioritizeRecommendations(
    ...
    return recommendations.sort((a, b) => {
      ...
      const impactWeight =
        (b.estimated_benefit || 0) - (a.estimated_benefit || 0);
    ...
    for (const forecast of state.forecasts.short_term) {
      ...
      if (currentInventory) {
        const daysUntilStockout =
          ...
          Math.max(forecast.predicted_demand, 1);
  ...
  private generateOverstockAlerts(state: StockAnalysisState): Alert[] {
    ...
    for (const inventory of state.raw_data.current_inventory) {
      const forecast = state.forecasts.medium_term.find(
      ...
      if (forecast) {
        ...
        const monthsOfStock =
          inventory.available_quantity / Math.max(monthlyDemand, 1);

        if (monthsOfStock > 6) {
          // Plus de 6 mois de stock
          alerts.push({
            id: this.generateAlertId(),
            type: 'OVERSTOCK',
            ...
            recommended_action:
              'Considérer promotion ou révision de la politique de commande',
          });
        }
      }
    }
    ...
    for (const forecast of state.forecasts.short_term) {
      if (forecast.accuracy_score < 0.5) {
        alerts.push({
          ...
          recommended_action:
            'Réviser modèle de prévision et qualité des données',
        });
      }
    }
    ...
    for (const [agentId, agent] of this.agents.entries()) {
      ...
      if (health.status === 'ERROR' || health.error_rate > 0.1) {
        alerts.push({
          ...
          recommended_action:
            "Vérifier configuration et données d'entrée de l'agent",
        });
      }
    }
  ...
  private calculateForecastAccuracy(
    ...
    const overallMape = this.calculateMean(
      allForecasts.map(f => (1 - f.accuracy_score) * 100)
    );
    const shortTermMape =
      state.forecasts.short_term.length > 0
        ? this.calculateMean(
            state.forecasts.short_term.map(f => (1 - f.accuracy_score) * 100)
          )
        : 100;
    const mediumTermMape =
      state.forecasts.medium_term.length > 0
        ? this.calculateMean(
            state.forecasts.medium_term.map(f => (1 - f.accuracy_score) * 100)
          )
        : 100;
  ...
  private calculateServiceMetrics(
    ...
    return {
      service_level:
        Math.max(0, 1 - stockoutAlerts.length / Math.max(totalProducts, 1)) *
        100,
  ...
  private calculateFinancialMetrics(
    ...
    return {
      inventory_turnover:
        monthlySales > 0 ? (monthlySales * 12) / totalInventoryValue : 0,
      days_of_inventory:
        totalInventoryValue > 0 ? totalInventoryValue / (monthlySales / 30) : 0,
  ...
  private calculateAIPerformance(
    ...
    return {
      alert_precision:
        totalAlerts > 0 ? (highPriorityAlerts / totalAlerts) * 100 : 0,
  ...
  private calculateInventoryStatus(state: StockAnalysisState) {
    ...
    const stockoutAlerts =
      state.alerts?.filter(a => a.type === 'STOCKOUT_RISK') || [];
    const overstockAlerts =
      state.alerts?.filter(a => a.type === 'OVERSTOCK') || [];

    return {
      products_below_min: stockoutAlerts.filter(
        a => a.severity === 'HIGH' || a.severity === 'CRITICAL'
      ).length,
      ...
      total_inventory_value: inventory.reduce(
        (sum, inv) => sum + (inv.available_quantity || 0) * 25,
        0
      ),
    };
  }
  ...
  private calculateTodayForecastConfidence(state: StockAnalysisState): number {
    const todayForecasts =
      state.forecasts?.short_term?.filter(
        f => this.daysBetween(new Date(), f.forecast_date) <= 1
      ) || [];
  ...
  private calculateInputSize(input: any): number {
    if (!input) return 0;
    return (
      (input.sales_history?.length || 0) +
      (input.current_inventory?.length || 0)
    );
  }
  ...
  private extractConfidenceIntervals(forecasts: any[]): {
  ...
  private calculateOverallConfidence(workflowResult: WorkflowResult): number {
    const performances = Object.values(
      workflowResult.performance_summary.agents_performance
    );
  ...
  private generateExecutionSummary(
    ...
    const executionTime =
      workflowResult.performance_summary.total_execution_time_ms;
    ...
    let summary =
      `Workflow Phase 3 exécuté en ${executionTime}ms avec ${agentsCount} agents. ` +
    ...
    if (marketIntelligence) {
      ...
      const competitorActions =
        marketIntelligence.competitor_analysis?.reduce(
          (sum: number, comp: any) =>
            sum + (comp.actions_detected?.length || 0),
          0
        ) || 0;
