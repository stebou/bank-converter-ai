...
export class ForecastingAgent extends BaseAgent {
  ...
  async execute(
    ...
    try {
      this.log('info', 'Starting forecasting analysis', {
        productsToForecast: new Set(
          input.historical_data.map(r => r.product_id)
        ).size,
      ...
      for (const [productId, salesData] of productData.entries()) {
        if (salesData.length < 10) {
          this.log('warn', `Insufficient data for product ${productId}`, {
            dataPoints: salesData.length,
          });
          continue;
        }

        const productPattern = input.demand_patterns.find(
      ...
      const overallPerformance =
        this.calculateOverallPerformance(modelPerformance);
      const recommendationQuality = this.assessRecommendationQuality(
        forecasts,
        input.demand_patterns
      );
      ...
      const output: ForecastingOutput = {
        ...
        model_selection_reasoning:
          this.generateModelSelectionReasoning(modelPerformance),
      };
      ...
      const executionTime = Date.now() - startTime;
      this.log('error', 'Forecasting failed', {
  ...
  private async forecastProduct(
    ...
    const selectedModels = this.selectOptimalModels(
      pattern,
      timeSeries.length,
      horizonDays
    );
    ...
    const ensembleForecasts = this.combineModelForecasts(
      modelResults,
      confidenceLevel
    );

    // Évaluation de la performance sur données historiques
    const performance = this.evaluateModelPerformance(
      timeSeries,
      selectedModels[0]
    );
  ...
  private fillMissingDays(
    ...
    while (currentDate <= endDate) {
      ...
      const value =
        dataMap.get(dateKey) || this.interpolateValue(currentDate, timeSeries);
  ...
  private interpolateValue(
    ...
    for (let i = 0; i < timeSeries.length - 1; i++) {
      if (
        timeSeries[i].date <= targetDate &&
        timeSeries[i + 1].date >= targetDate
      ) {
        before = timeSeries[i];
    ...
    const ratio =
      (targetDate.getTime() - before.date.getTime()) /
  ...
  private async predictValue(
    ...
    switch (modelType) {
      case 'moving_average':
        ...
        uncertainty = this.calculateStandardDeviation(
      ...
      case 'seasonal_naive':
        ...
        const seasonalValues = values.filter(
          (_, i) => i % seasonLength === seasonalIndex
        );
    ...
    if (externalFactors && externalFactors.length > 0) {
      const adjustment = this.applyExternalFactors(
        predictedValue,
        externalFactors,
        daysAhead
      );
  ...
  private calculateLinearTrend(values: number[]): {
    ...
    const n = values.length;
    if (n < 2)
      return { predict: () => values[0] || 0, uncertainty: 0, rsquared: 0 };
    ...
    const ssRes = y.reduce(
      (acc, yi, i) => acc + Math.pow(yi - (slope * i + intercept), 2),
      0
    );
  ...
  private holtWinters(
    values: number[],
    daysAhead: number
  ): { forecast: number; uncertainty: number; confidence: number } {
    // Implémentation simplifiée de Holt-Winters
    if (values.length < 14) {
      return {
        forecast: this.calculateMean(values),
        uncertainty: this.calculateStandardDeviation(values),
        confidence: 0.5,
      };
    }
    ...
    for (let i = seasonLength; i < values.length; i++) {
      ...
      level =
        alpha * (values[i] / seasonal[seasonalIdx]) +
        (1 - alpha) * (prevLevel + trend);
      trend = beta * (level - prevLevel) + (1 - beta) * trend;
      seasonal[seasonalIdx] =
        gamma * (values[i] / level) + (1 - gamma) * seasonal[seasonalIdx];
    }

    // Prévision
    const forecastSeasonalIdx = (values.length + daysAhead - 1) % seasonLength;
    const forecast =
      (level + trend * daysAhead) * seasonal[forecastSeasonalIdx];
  ...
  private crostonMethod(
    ...
    const uncertainty =
      this.calculateStandardDeviation(nonZeroValues) / Math.sqrt(avgInterval);
  ...
  private applyExternalFactors(
    ...
    for (const factor of factors) {
      const daysDiff = Math.abs(
        (factor.date.getTime() - targetDate.getTime()) / (1000 * 60 * 60 * 24)
      );

      if (daysDiff <= 7) {
        // Facteur actif dans les 7 jours
  ...
  private combineModelForecasts(
    ...
    for (let day = 0; day < numDays; day++) {
      ...
      // Moyenne pondérée
      const weightedPrediction = dayForecasts.reduce(
        (sum, forecast, i) =>
          sum + forecast.predicted_demand * (weights[i] / totalWeight),
        0
      );

      // Calcul de l'incertitude combinée
      const variance = dayForecasts.reduce(
        (sum, forecast, i) =>
          sum +
          Math.pow(forecast.predicted_demand - weightedPrediction, 2) *
  ...
  private calculateConfidenceInterval(
    ...
    return [Math.max(0, prediction - margin), prediction + margin];
  }

  // Méthodes utilitaires

  private groupDataByProduct(
    salesData: SalesRecord[]
  ...
  private calculateOverallPerformance(
    modelPerformances: ForecastMetrics[]
  ): ForecastMetrics {
    ...
    return {
      ...
      tracking_signal: this.calculateMean(
        modelPerformances.map(p => p.tracking_signal)
      ),
    };
  }
  ...
  private assessRecommendationQuality(
    ...
    for (const [productId, productForecastList] of productForecasts.entries()) {
      ...
      const avgAccuracy = this.calculateMean(
        productForecastList.map(f => f.accuracy_score)
      );
      const consistencyScore = this.calculateForecastConsistency(
        productForecastList,
        pattern
      );

      qualityScore +=
        (avgAccuracy * 0.7 + consistencyScore * 0.3) *
        productForecastList.length;
  ...
  private calculateForecastConsistency(
    forecasts: ForecastResult[],
    ...
    switch (pattern.pattern_type) {
      case 'STABLE':
        ...
        const cv =
          this.calculateStandardDeviation(values) / this.calculateMean(values);
        return Math.max(0, 1 - cv);

      case 'TRENDING':
        // Pour un pattern avec tendance, vérifier la monotonie
        const trend = this.detectMonotonicity(values);
        return pattern.trend_direction === 'UP'
          ? trend.upward
          : pattern.trend_direction === 'DOWN'
            ? trend.downward
            : trend.stable;
  ...
  private detectMonotonicity(values: number[]): {
    upward: number;
    downward: number;
    stable: number;
  } {
