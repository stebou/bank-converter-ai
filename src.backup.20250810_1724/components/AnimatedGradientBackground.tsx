...
const AnimatedGridBackground = () => {
  ...
  useEffect(() => {
    ...
    const newSquares: Square[] = Array.from({ length: numSquares }).map(
      (_, i) => ({
        id: `square-${i}`,
        x: Math.random() * 85,
        y: Math.random() * 85,
    ...
    const newHorizontalLines: HorizontalLine[] = Array.from({
      length: numHorizontalLines,
    }).map((_, i) => ({
      id: `h-line-${i}`,
    ...
    const newVerticalLines: VerticalLine[] = Array.from({
      length: numVerticalLines,
    }).map((_, i) => ({
      id: `v-line-${i}`,
      ...
      >
        ...
        <g>
          {horizontalLines.map(line => (
            <FastHorizontalLine key={line.id} y={line.y} delay={line.delay} />
          ))}
        </g>
        ...
        <g>
          {verticalLines.map(line => (
            <FastVerticalLine key={line.id} x={line.x} delay={line.delay} />
          ))}
        </g>
