  ...
  try {
    ...
    if (file.type === 'application/pdf') {
      ...
      try {
        ...
        const pdfProcessingResult = await processPdfNative(
          pdfBuffer,
          file.name
        );
  ...
  "analysisMethod": ${base64Image ? '"hybrid_text_and_vision"' : '"text_analysis_integrated"'}
          ...
          if (aiResponse) {
            try {
              ...
              const bankName =
                pdfProcessingResult.metadata?.detected_bank ||
                analysis.bankName ||
                'Document Financier';
              console.log('[VALIDATE_DOCUMENT] Final bank name determination:');
              console.log(
                '[VALIDATE_DOCUMENT] - From PDF processing:',
                pdfProcessingResult.metadata?.detected_bank
              );
              console.log(
              ...
              const generatedTransactions = await extractTransactionsWithAI(
                extractedText,
                base64Image
              );
              console.log(
