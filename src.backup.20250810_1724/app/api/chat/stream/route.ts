...
export async function POST(req: NextRequest) {
  try {
    ...
    const { message, conversationId, documents, conversationHistory } =
      await req.json();
...
- ${doc.originalName} (${doc.bankDetected || 'Banque non identifiée'})
  ...
  Transactions récentes: ${doc.transactions
    .slice(0, 5)
    .map(
      t =>
        ...
        try {
          ...
          await prisma.conversation.update({
            ...
            data: {
              ...
              title:
                conversation.title ===
                `Nouvelle conversation ${conversation.createdAt.toLocaleDateString('fr-FR')}`
          ...
          const finalData = JSON.stringify({
            ...
            messageId,
            referencedDocuments:
              referencedDocuments.length > 0 ? referencedDocuments : null,
          });
          ...
          if (error instanceof Error) {
            if (
              ...
              errorMessage =
                '⚠️ Quota OpenAI dépassé. Veuillez réessayer plus tard.';
